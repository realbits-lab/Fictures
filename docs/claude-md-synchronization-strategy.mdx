...
- ✅ **High-level overviews** with links to detailed documentation
- ✅ **Essential commands** and development workflows
- ✅ **Quick reference** information for common tasks
- ✅ **Critical reminders** (e.g., environment setup, naming conventions)
- ✅ **Links to authoritative documentation** for deep dives

**What CLAUDE.md SHOULD NOT contain:**
- ❌ **Detailed API specifications** → Link to `docs/novels/novels-development.md`
- ❌ **Complete system prompts** → Link to relevant docs
- ❌ **Exhaustive feature descriptions** → Link to specification files
- ❌ **Implementation details** → Point to source code locations
...
**Example:**
- ❌ Don't duplicate entire novel generation pipeline in CLAUDE.md
- ✅ Do provide a compact summary with links to `docs/novels/novels-specification.mdx` and `docs/novels/novels-development.md`
...
- [ ] **Minor feature additions** → Update feature-specific docs only
- [ ] **Bug fixes** → Update implementation code only
- [ ] **API endpoint changes** → Update API documentation only
- [ ] **UI component changes** → Update component docs only
- [ ] **Performance optimizations** → Update optimization docs only
...
- **Yes** → Update CLAUDE.md + relevant docs
- **No** → Update only the specific documentation files
...
- **📖 Specification**: [docs/[feature]/specification.mdx](docs/[feature]/specification.mdx) - Core concepts
- **🔧 Development**: [docs/[feature]/development.md](docs/[feature]/development.md) - Implementation guide
- **🧪 Testing**: [docs/[feature]/testing.mdx](docs/[feature]/testing.mdx) - Testing strategies
...
- [ ] Check if any new environment variables added → Update CLAUDE.md
- [ ] Check if major features added → Update relevant sections
...
**Do NOT update for:**
- New columns added to existing tables → Update schema docs only
- Index changes → Update database optimization docs only
...
src/
├── app/
│   ├── studio/            # Story creation workspace
...
### ❌ Anti-Pattern 1: Duplicating Detailed Specs
...
**Quick Reference:**
- 9-phase pipeline: Story Summary → Characters → Settings → ... → Images
- Generation time: 5-25 minutes
- API: `/studio/api/generation/*`
```

### ❌ Anti-Pattern 2: Inline Code Duplication
...
**Output:** 4 variants (AVIF, JPEG × mobile 1x/2x)
```
...
- [ ] Does this change affect project structure? → Update structure section
- [ ] Does this add/change environment variables? → Update env section
- [ ] Does this change development workflow? → Update commands section
- [ ] Does this add major feature? → Add quick reference + link to docs
...
```
docs/comics/
├── comics-specification.mdx   (Core concepts)
├── comics-development.md      (API specs)
└── comics-testing.mdx         (Testing guide)
```
...
**Complete Documentation:**
- **📖 Specification**: [docs/comics/comics-specification.mdx](docs/comics/comics-specification.mdx)
- **🔧 Development**: [docs/comics/comics-development.md](docs/comics/comics-development.md)
- **🧪 Testing**: [docs/comics/comics-testing.mdx](docs/comics/comics-testing.mdx)

**Quick Reference:**
- **API**: `/api/comic/generate-panels`
- **Process**: Scene → Screenplay → Panel Layout → Image Generation
