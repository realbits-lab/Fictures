...
**Fix**: Added ID mapping in `src/app/studio/api/novels/generate/route.ts`:
```typescript
// Parts
...
**For New Stories:**
1. Generate a new story using `/studio/api/novels/generate`
...
**Generation Code:**
- `src/app/studio/api/novels/generate/route.ts` - Fixed ID mapping for parts, chapters, scenes
