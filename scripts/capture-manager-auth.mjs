...
/**
 * Capture Manager Authentication
 *
 * This script logs in as manager@fictures.xyz and captures the session cookies
 * to .auth/user.json for use in other scripts.
 *
 * Prerequisites:
 * - Development server must be running on port 3000
 * - Manager account must exist with credentials from .auth/user.json
 *
 * Usage:
 *   dotenv --file .env.local run node scripts/capture-manager-auth.mjs
 */
...
async function captureManagerAuth() {
  console.log('\n' + '='.repeat(70));
  console.log('🔐 CAPTURE MANAGER AUTHENTICATION');
  ...
  if (!authData.profiles?.manager) {
    console.error('❌ Manager profile not found in .auth/user.json');
    process.exit(1);
  }

  const manager = authData.profiles.manager;
  console.log(`✓ Found manager profile: ${manager.email}\n`);

  // Check if manager has credentials
  if (!manager.email || !manager.password) {
    console.error('❌ Manager profile missing email or password');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`   Password: ${manager.password.replace(/./g, '*')}\n`);
    ...
    if (await googleButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      console.log(`   2. Log in with ${manager.email}`);
      ...
      await emailInput.fill(manager.email);
      await passwordInput.fill(manager.password);
    ...
    authData.profiles.manager.cookies = relevantCookies;
    authData.profiles.manager.origins = [
      {
        origin: BASE_URL,
    ...
    console.log('='.repeat(70));
    console.log('\n📊 Summary:');
    console.log(`   Profile: manager (${manager.email})`);
    ...
    console.log('✨ You can now use manager authentication in other scripts!\n');
