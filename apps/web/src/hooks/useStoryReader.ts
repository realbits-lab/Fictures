...
const storyETagCache = new Map<
    string,
    { data: StoryReaderResponse; etag: string; timestamp: number }
>();
...
export function useStoryReader(
    ...
    const { data, error, isLoading, isValidating, mutate } =
        usePersistedSWR<StoryReaderResponse>(
            shouldFetch ? `/studio/api/story/${storyId}/read` : null,
            fetcher,
            {
    ...
    const availableChapters = useMemo(() => {
        ...
        const filteredChapters = allChapters.filter(
            (chapter) => data.isOwner || chapter.status === "published",
        );
    ...
    return {
        prefetch: async (storyId: string) => {
            try {
                ...
                await fetch(`/studio/api/story/${storyId}/read`, {
                    credentials: "include",
                });
            } catch (error) {
