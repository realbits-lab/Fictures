...
export function useStoryWriter(storyId: string | null): UseStoryWriterReturn {
    ...
    const { data, error, isLoading, isValidating, mutate } =
        usePersistedSWR<StoryWriterResponse>(
            shouldFetch ? `/studio/api/story/${storyId}/write` : null,
            fetcher,
            {
    ...
    const allScenes = useMemo(() => {
        ...
        const scenesFromChapters = allChapters.flatMap(
...
export function usePrefetchStoryWriter() {
    return {
        prefetch: async (storyId: string) => {
            try {
                await fetch(`/studio/api/story/${storyId}/write`, {
                    credentials: "include",
                });
            } catch (error) {
                console.debug(
                    "Prefetch failed for story writer:",
                    storyId,
                    error,
                );
            }
