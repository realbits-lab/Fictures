...
export const POST = requireScopes("stories:write")(
    withAuthentication(async (request: NextRequest): Promise<Response> => {
        ...
        console.log(
            ...
            `type: ${auth.type}, user: ${auth.email}`,
        );
                    ...
                    try {
                        ...
                        const onProgress = async (progress: ProgressData) => {
                            ...
                            if (progress.phase === "story_complete") {
                                ...
                                progress.phase === "characters_complete"
                            ) {
                        ...
                        if (result.story.genre) {
                            ...
                            const exactMatch = validGenres.find(
                                (g) =>
                                    g.toLowerCase() === aiGenre.toLowerCase(),
                            );
                            if (exactMatch) {
                        ...
                        if (result.parts.length > 0) {
                            ...
                            const partRecords = result.parts.map(
                                (part, _index) => {
                                    ...
                                    const mappedCharacterArcs =
                                        part.characterArcs.map((arc: any) => {
                                            console.log(
                            ...
                            const sceneRecords = result.scenes.map(
                                (scene, index) => {
                                    ...
                                    const mappedSettingId = scene.settingId
                                        ? settingIdMap.get(scene.settingId) ||
                                          null
                                        : null;
                                    ...
                                    return insertSceneSchema.parse({
                                        ...
                                        title:
                                            scene.title || `Scene ${index + 1}`, // Fallback title if missing
                        ...
                        if (!skipImages) {
                            ...
                            try {
                                ...
                                if (storyCoverResponse.ok) {
                                    ...
                                    await db
                                        ...
                                        .where(
                                            eq(stories.id, generatedStoryId!),
                                        );
                                } else {
                                    const error =
                                        await storyCoverResponse.json();
                            ...
                            for (let i = 0; i < result.settings.length; i++) {
                                ...
                                const settingDbId = settingIdMap.get(
                                    setting.id,
                                );
