...
export async function POST(request: NextRequest) {
    try {
        ...
        // Run validation
        if (
            validatedRequest.analysisType === "validation" ||
            validatedRequest.analysisType === "both"
        ) {
            const validationResult = validateStoryStructure(
            ...
            if (!validatedRequest.options?.includeWarnings) {
                ...
                validationResult.parts = validationResult.parts.map((p) => ({
                    ...p,
                    warnings: [],
                }));
                validationResult.chapters = validationResult.chapters.map(
                    (c) => ({
                        ...c,
                        warnings: [],
                    }),
                );
                validationResult.scenes = validationResult.scenes.map((s) => ({
                    ...s,
                    warnings: [],
                }));
                validationResult.characters = validationResult.characters.map(
                    (c) => ({
                        ...c,
                        warnings: [],
                    }),
                );
                validationResult.settings = validationResult.settings.map(
                    (s) => ({
                        ...s,
                        warnings: [],
                    }),
            ...
            results.validation = {
                ...
                details:
                    validatedRequest.options?.detailLevel !== "basic"
                        ? validationResult
                        : undefined,
        ...
        // Run evaluation
        if (
            validatedRequest.analysisType === "evaluation" ||
            validatedRequest.analysisType === "both"
        ) {
            if (validatedRequest.options?.includeAIFeedback) {
                const evaluationResult = await evaluateStoryContent(
                    validatedRequest.data,
                );
                ...
                if (validatedRequest.options?.detailLevel === "basic") {
                    results.evaluation = {
                        overallScore:
                            evaluationResult.storyEvaluation?.overallScore || 0,
                        summary:
                            evaluationResult.storyEvaluation?.summary ||
                            "No evaluation available",
                        keyStrengths:
                            evaluationResult.storyEvaluation?.keyStrengths ||
                            [],
                        prioritizedImprovements:
                            evaluationResult.storyEvaluation
                                ?.prioritizedImprovements || [],
                    };
                } else {
        ...
        if (
            validatedRequest.options?.generateReport &&
            validatedRequest.analysisType === "both"
        ) {
            results.report = generateComprehensiveReport(
                results.validation,
                results.evaluation,
            );
        }
        ...
        if (
            validatedRequest.options?.autoImprove &&
            validatedRequest.analysisType === "both"
        ) {
            try {
                const improvementResult = await improveStoryContent({
                    ...
                    options: {
                        updateLevel:
                            validatedRequest.options.improvementLevel ||
                            "moderate",
                ...
                results.improvements = {
                    ...
                    message:
                        "Improvements generated. Use /api/story-update with autoApply: true to apply changes.",
                };
                ...
                results.improvements = {
                    ...
                    message:
                        "Analysis completed but improvement generation failed",
                };
            }
        }
...
function generateValidationSummary(validationResult: any): string {
    ...
    if (validationResult.story.isValid) {
        components.push(
            `Story: ${validationResult.story.stats.completenessPercentage}% complete`,
        );
    } else {
        components.push(
            `Story: ${validationResult.story.errors.length} errors`,
        );
    }

    if (validationResult.parts.length > 0) {
        const validParts = validationResult.parts.filter(
            (p: any) => p.isValid,
        ).length;
        components.push(
            `Parts: ${validParts}/${validationResult.parts.length} valid`,
        );
    }

    if (validationResult.chapters.length > 0) {
        const validChapters = validationResult.chapters.filter(
            (c: any) => c.isValid,
        ).length;
        components.push(
            `Chapters: ${validChapters}/${validationResult.chapters.length} valid`,
        );
    }

    if (validationResult.scenes.length > 0) {
        const validScenes = validationResult.scenes.filter(
            (s: any) => s.isValid,
        ).length;
        components.push(
            `Scenes: ${validScenes}/${validationResult.scenes.length} valid`,
        );
    }

    if (validationResult.characters.length > 0) {
        const validCharacters = validationResult.characters.filter(
            (c: any) => c.isValid,
        ).length;
        components.push(
            `Characters: ${validCharacters}/${validationResult.characters.length} valid`,
        );
    }

    if (validationResult.settings.length > 0) {
        const validSettings = validationResult.settings.filter(
            (s: any) => s.isValid,
        ).length;
        components.push(
            `Settings: ${validSettings}/${validationResult.settings.length} valid`,
        );
    }
        ...
        structure: {
            ...
            averageScenesPerChapter:
                data.chapters?.length > 0
                    ? Math.round(
                          (data.scenes?.length || 0) / data.chapters.length,
                      )
                    : 0,
        },
        worldBuilding: {
            ...
            mainCharacters:
                data.characters?.filter((c: any) => c.isMain).length || 0,
    ...
    return {
        executiveSummary: {
            ...
            evaluationScore:
                ...
                evaluation?.storyEvaluation?.overallScore ||
                0,
            readiness: calculateReadiness(validation, evaluation),
        },
        strengths: [
            ...(validation?.overallValid
                ? ["Story structure is technically valid"]
                : []),
            ...(evaluation?.keyStrengths ||
                evaluation?.storyEvaluation?.keyStrengths ||
                []),
        ],
        criticalIssues: [
            ...(validation?.totalErrors > 0
                ? [`${validation.totalErrors} validation errors need fixing`]
                : []),
            ...(evaluation?.prioritizedImprovements ||
                evaluation?.storyEvaluation?.prioritizedImprovements ||
                []),
        ],
...
function calculateReadiness(validation: any, evaluation: any): string {
    ...
    if (evaluationScore < 50)
        return "Early Draft - Significant improvements needed";
...
function generateRecommendations(validation: any, evaluation: any): string[] {
    ...
    if (validation?.totalErrors > 0) {
        recommendations.push(
            "Priority: Fix all validation errors before proceeding",
        );
    }

    if (validation?.totalWarnings > 5) {
        recommendations.push(
            `Address ${validation.totalWarnings} warnings to improve quality`,
        );
    }
    ...
    if (evalScore < 70) {
        recommendations.push(
            "Focus on fundamental story elements and structure",
        );
    }

    if (evaluation?.crossReferenceAnalysis?.plotHoles?.length > 0) {
        recommendations.push(
            "Resolve identified plot holes for narrative consistency",
        );
    }

    if (
        evaluation?.characterEvaluations?.some((c: any) => c.consistency < 60)
    ) {
        recommendations.push(
            "Improve character consistency throughout the story",
        );
    }
