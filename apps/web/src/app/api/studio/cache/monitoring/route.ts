    ...
    try {
        ...
        const response = {
            ...
            alerts: {
                ...
                critical: health.alerts.filter((a) => a.severity === "critical")
                    .length,
                error: health.alerts.filter((a) => a.severity === "error")
                    .length,
                warning: health.alerts.filter((a) => a.severity === "warning")
                    .length,
                list: health.alerts,
            },
        };
...
export async function POST(request: NextRequest) {
    try {
        ...
        console.error(
            "[Cache Monitoring API] Error acknowledging alert:",
            error,
        );
        return NextResponse.json(
