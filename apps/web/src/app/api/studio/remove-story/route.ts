...
export async function POST(request: NextRequest) {
    ...
    try {
            ...
            return Response.json(
                {
                    ...
                    message:
                        "You need writer or manager permissions to remove stories",
                },
                { status: 403 },
            );
        }
            ...
            return Response.json(
                {
                    ...
                    message:
                        'Must send { "confirm": true } to proceed with removal',
        ...
        if (!hasAdminScope) {
            ...
            console.log(
                "✅ [REMOVE STORY] Admin access - ownership check bypassed",
            );
        }
        ...
        deletionReport.database.sceneViews =
            Number(sceneViewsCount[0]?.count) || 0;
        console.log(
        ...
        console.log(
            `   ✓ Deleted ${deletionReport.database.chapters} chapters`,
        );
        ...
        deletionReport.database.characters =
            Number(charactersCount[0]?.count) || 0;
        console.log(
        ...
        console.log(
            `   ✓ Deleted ${deletionReport.database.settings} settings`,
        );
        ...
        do {
            ...
            if (urls.length > 0) {
                batchCount++;
                console.log(
                    `   Batch ${batchCount}: Deleting ${urls.length} files...`,
                );
