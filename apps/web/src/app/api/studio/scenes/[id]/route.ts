...
export async function PATCH(
    ...
    try {
        ...
        if (validatedData.content && validatedData.content.trim().length > 0) {
            ...
            const allScenesHaveContent = chapterScenes.every(
            ...
            if (allScenesHaveContent && chapter.status === "writing") {
                ...
                console.log(
                    `🎉 Chapter "${chapter.title}" automatically published - all scenes have content`,
                );
            }
        }
        ...
        return NextResponse.json(
            {
                scene: updatedScene,
                chapterAutoPublished:
                    validatedData.content &&
                    validatedData.content.trim().length > 0,
            },
            {
