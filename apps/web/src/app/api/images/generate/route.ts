...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!prompt && !autoPrompt) {
            return new Response(
                JSON.stringify({
                    error: "Either prompt or autoPrompt must be provided",
                }),
                {
                    status: 400,
                    headers: { "Content-Type": "application/json" },
...
export async function GET() {
    return new Response(
        JSON.stringify({
            ...
            summary:
                "Generate story illustrations using DALL-E 3 with 16:9 aspect ratio (1792x1024)",
            ...
            parameters: {
                prompt: {
                    ...
                    summary:
                        "Image generation prompt (required if autoPrompt is false)",
                    example:
                        "A mysterious forest at twilight with ancient trees, cinematic composition",
                },
                storyId: {
                    type: "string",
                    required: false,
                ...
                style: {
                    ...
                    summary:
                        "Image style - vivid for hyper-real, natural for realistic",
                },
                quality: {
                    type: "string",
                    required: false,
                ...
                autoPrompt: {
                    ...
                    default: false,
                    summary:
                        "Auto-generate prompt from story context (requires storyId)",
                },
            },
            response: {
                success: true,
