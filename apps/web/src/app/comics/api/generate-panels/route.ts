...
export async function POST(request: NextRequest) {
	try {
		...
		if (!hasRequiredScope(authResult, "ai:use")) {
			return new Response(
				JSON.stringify({
					error: "Insufficient permissions. Required scope: ai:use",
				}),
				{
					status: 403,
					headers: { "Content-Type": "application/json" },
		...
		if (
			...
			return new Response(
				JSON.stringify({ error: "targetPanelCount must be between 1 and 12" }),
				{
					status: 400,
					headers: { "Content-Type": "application/json" },
				},
		...
		if (story.authorId !== authResult.user.id) {
			return new Response(
				JSON.stringify({ error: "Forbidden - You do not own this story" }),
				{
					status: 403,
					headers: { "Content-Type": "application/json" },
				},
