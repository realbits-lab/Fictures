	...
	try {
		...
		if (
			...
			if (!validatedRequest.options?.includeWarnings) {
				...
				validationResult.chapters = validationResult.chapters.map((c) => ({
					...c,
					warnings: [],
				}));
				...
				validationResult.characters = validationResult.characters.map((c) => ({
					...c,
					warnings: [],
				}));
				validationResult.settings = validationResult.settings.map((s) => ({
					...s,
					warnings: [],
				}));
			}
				...
				if (validatedRequest.options?.detailLevel === "basic") {
					results.evaluation = {
						...
						prioritizedImprovements:
							evaluationResult.storyEvaluation?.prioritizedImprovements || [],
					};
				} else {
		...
		if (
			...
			try {
				const improvementResult = await improveStoryContent({
					...
					options: {
						updateLevel:
							validatedRequest.options.improvementLevel || "moderate",
...
function calculateReadiness(validation: any, evaluation: any): string {
	...
	const evaluationScore =
		evaluation?.overallScore || evaluation?.storyEvaluation?.overallScore || 0;
