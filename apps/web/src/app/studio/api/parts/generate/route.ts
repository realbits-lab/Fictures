	...
	try {
		...
		if (!(story as any).content || typeof (story as any).content !== "string") {
			return new Response(
				JSON.stringify({
					error:
						"Story content is required for part generation. Please regenerate the story first.",
				}),
		...
		for (let i = 0; i < partSpecs.length; i++) {
			...
			const [existingPart] = await db
				...
				.from(parts)
				.where(
					and(eq(parts.storyId, storyId), eq(parts.orderIndex, partSpec.part)),
				);
			...
			if (existingPart) {
				...
				const [updatedPart] = await db
					...
					.set({
						...
						...(JSON.stringify(partSpec)
							? ({ content: JSON.stringify(partSpec) } as any)
							: {}),
				...
				const partId = await RelationshipManager.addPartToStory(storyId, {
					title: partSpec.title,
