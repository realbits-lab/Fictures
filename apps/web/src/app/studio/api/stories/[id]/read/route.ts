...
export async function GET(
    ...
    try {
        ...
        if (
            storyWithStructure.status !== "published" &&
            storyWithStructure.userId !== session?.user?.id
        ) {
            return NextResponse.json(
        ...
        const response = {
            ...
            metadata: {
                ...
                totalChapters: [
                    ...storyWithStructure.parts.flatMap(
                        (part) => part.chapters,
                    ),
        ...
        const headers = new Headers({
            ...
            "Cache-Control":
                storyWithStructure.status === "published" && !isOwner
