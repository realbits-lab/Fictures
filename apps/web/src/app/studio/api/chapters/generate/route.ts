...
import { chapters, parts, stories } from "@/lib/db/schema";
	...
	try {
		...
		if (!(part as any).content || typeof (part as any).content !== "string") {
			return new Response(
				JSON.stringify({
					error:
						"Part specification data is required. Please generate part specifications first.",
				}),
		...
		if (!(story as any).content || typeof (story as any).content !== "string") {
			return new Response(
				JSON.stringify({
					error:
						"Story content is required. Please regenerate the story first.",
				}),
				{ status: 400, headers: { "Content-Type": "application/json" } },
			);
		}
		...
		console.log(
			"📖 Chapter specifications generated, creating database entries...",
		);
