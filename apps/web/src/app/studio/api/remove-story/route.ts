...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!hasWriteScope && !hasAdminScope) {
      console.log(`❌ [REMOVE STORY] Insufficient permissions for user: ${authResult.user.email}`);
    ...
    console.log(`✅ [REMOVE STORY] Authenticated as: ${authResult.user.email} (${authResult.type})`);
    ...
    console.log('⚠️  [REMOVE STORY] Confirmation received - proceeding with DESTRUCTIVE removal\n');
    ...
    console.log(`📖 [REMOVE STORY] Found story: "${story.title}" (ID: ${story.id})`);
    ...
    await db.delete(sceneViews).where(
      sql`${sceneViews.sceneId} IN (SELECT id FROM ${scenes} WHERE ${scenes.chapterId} IN (SELECT id FROM ${chapters} WHERE ${chapters.storyId} = ${storyId}))`
    );
    ...
    await db.delete(comicPanels).where(
      sql`${comicPanels.sceneId} IN (SELECT id FROM ${scenes} WHERE ${scenes.chapterId} IN (SELECT id FROM ${chapters} WHERE ${chapters.storyId} = ${storyId}))`
    );
