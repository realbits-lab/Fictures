...
const StoryUpdateRequestSchema = z.object({
  ...
  options: z.object({
    updateLevel: z.enum(['conservative', 'moderate', 'aggressive']).optional().default('moderate'),
    preserveUserContent: z.boolean().optional().default(true),
    focusAreas: z.array(z.enum(['structure', 'character', 'world', 'pacing', 'dialogue'])).optional(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const improvementResult = await improveStoryContent({
      ...
      options: {
        updateLevel: validatedRequest.options?.updateLevel || 'moderate',
        preserveUserContent: validatedRequest.options?.preserveUserContent ?? true,
    ...
    return NextResponse.json({
      ...
      mode: 'preview',
      result: improvementResult,
      message: 'Improvements generated. Review and apply manually or set autoApply: true'
    });
...
async function applyImprovements(
  ...
  try {
    ...
    for (const part of improvementResult.improved.parts) {
      const changeLog = improvementResult.changes.parts.find((c: any) => c.id === part.id);
    ...
    for (const chapter of improvementResult.improved.chapters) {
      const changeLog = improvementResult.changes.chapters.find((c: any) => c.id === chapter.id);
      if (changeLog && changeLog.fieldsUpdated.length > 0) {
        const chapterUpdate = {
          ...chapter,
          updatedAt: new Date().toISOString()
    ...
    for (const scene of improvementResult.improved.scenes) {
      const changeLog = improvementResult.changes.scenes.find((c: any) => c.id === scene.id);
      if (changeLog && changeLog.fieldsUpdated.length > 0) {
        const sceneUpdate = {
          ...scene,
          updatedAt: new Date().toISOString()
    ...
    for (const character of improvementResult.improved.characters) {
      const changeLog = improvementResult.changes.characters.find((c: any) => c.id === character.id);
      if (changeLog && changeLog.fieldsUpdated.length > 0) {
    ...
    for (const setting of improvementResult.improved.settings) {
      const changeLog = improvementResult.changes.settings.find((c: any) => c.id === setting.id);
      if (changeLog && changeLog.fieldsUpdated.length > 0) {
        const settingUpdate = {
          ...setting,
          updatedAt: new Date().toISOString()
