...
export async function POST(request: NextRequest) {
	try {
		...
		if (!chapterSpecification) {
			return new Response(
				JSON.stringify({
					error: "Chapter specification is required for scene generation",
				}),
		...
		console.log(
			"📖 Scene specifications generated, creating database entries...",
		);
		...
		for (let i = 0; i < sceneSpecs.length; i++) {
			...
			const sceneId = await RelationshipManager.addSceneToChapter(chapter.id, {
...
export async function GET() {
	return new Response(
		JSON.stringify({
			...
			usage:
				'POST with { "chapterId": "chapter-id", "sceneCount": 3, "chapterSpecification": {...} }',
