		...
		const stream = new ReadableStream({
			async start(controller) {
				try {
					...
					let toneValue: "hopeful" | "dark" | "bittersweet" | "satirical" =
						"hopeful"; // default
					if (result.story.tone) {
						...
						if (
							["hopeful", "dark", "bittersweet", "satirical"].includes(
								firstTone,
							)
						) {
						...
						if (exactMatch) {
							...
							if (genreMapping[lowerGenre]) {
								...
								for (const validGenre of validGenres) {
									if (
										lowerGenre.includes(validGenre.toLowerCase()) ||
										validGenre.toLowerCase().includes(lowerGenre)
									) {
					...
					if (result.characters.length > 0) {
						...
						const characterRecords = result.characters.map((char) => {
							...
							if (
								...
								mappedRelationships = {};
								for (const [tempCharId, relationshipData] of Object.entries(
									char.relationships,
								)) {
						...
						const chapterRecords = result.chapters.map((chapter, index) => {
							...
							return {
								...
								characterId: chapter.characterId
									? characterIdMap.get(chapter.characterId) || null
									: null,
						...
						const sceneRecords = result.scenes.map((scene, index) => {
							...
							return {
								...
								dialogueVsDescription:
									scene.dialogueVsDescription || "balanced",
										...
										createSSEMessage({
											...
											data: {
												...
												percentage: Math.round(
													((completedImages + 1) / totalImages) * 100,
												),
											},
										}),
									),
