	...
	try {
		...
		return NextResponse.json({
			...
			metadata: {
				...
				active: formattedKeys.filter((key) => key.isActive && !key.isExpired)
					.length,
...
export async function PATCH(request: NextRequest) {
	return NextResponse.json(
		{
			error:
				"Use PATCH /api/settings/api-keys/{id} to update specific API keys",
		},
		{ status: 405 },
	);
}
...
export async function DELETE(request: NextRequest) {
	return NextResponse.json(
		{
			error:
				"Bulk deletion not supported. Use DELETE /api/settings/api-keys/{id}",
		},
		{ status: 405 },
	);
}
