  ...
  try {
    ...
    if (redis) {
      ...
      if (cachedData) {
        ...
        console.log(`[Cache Test API] Redis HIT for story ${storyId} (${duration}ms)`);
      ...
      console.log(`[Cache Test API] Redis not available, fetching from database`);
    }
    ...
    const allScenes = chapterIds.length > 0
      ? await db.query.scenes.findMany({
          where: sql`${scenes.chapterId} IN (${sql.join(chapterIds.map(id => sql`${id}`), sql`, `)})`,
