...
interface CacheStats {
	...
	byPage?: Record<
		string,
		{ size: number; entries: number; lastUpdated?: string }
	>;
}

export function CacheManagerWidget() {
	const { clearAllCache, getCacheStats, clearPageCache, invalidatePageCache } =
		useCacheManagement();
	...
	const handleClearAll = async () => {
		if (
			window.confirm(
				"Are you sure you want to clear all cached data? This will refresh the page.",
			)
		) {
			setIsLoading(true);
			clearAllCache();
			...
			<CardContent className="space-y-6">
								...
								>
									<div className="flex items-center gap-3">
										...
										<div>
											...
											<div className="text-sm text-gray-600 dark:text-gray-400">
												TTL: {ttlMinutes}min • {pageStats?.entries || 0} entries
												• {formatBytes(pageStats?.size || 0)}
											</div>
										</div>
									</div>
				...
				<div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
					...
					<div className="text-sm text-blue-800 dark:text-blue-200 space-y-1">
						<p>
							• <strong>Writing:</strong> 30min TTL (personal stories change
							infrequently)
						</p>
						<p>
							• <strong>Reading:</strong> 10min TTL (public content updates
							regularly)
						</p>
						<p>
							• <strong>Community:</strong> 5min TTL (active discussions change
							frequently)
						</p>
						<p>
							• <strong>Analytics:</strong> 2min TTL (metrics update frequently)
						</p>
						<p>
							• <strong>Publish:</strong> 1hr TTL (publishing schedules change
							moderately)
						</p>
						<p>
							• <strong>Settings:</strong> 24hr TTL (preferences rarely change)
						</p>
					</div>
				</div>
