						...
						try {
							...
							switch (data.phase) {
								...
								case "characters_progress":
									...
									if (
										data.data?.currentCharacter !== undefined &&
										...
										const percentage =
											...
											Math.round((characterNum / totalCharacters) * 100);
								...
								case "settings_progress":
									...
									if (
										data.data?.currentSetting !== undefined &&
										...
										const percentage =
											data.data.percentage ||
											Math.round((settingNum / totalSettings) * 100);
								...
								case "scene_summaries_progress":
									if (data.data?.currentChapter && data.data?.totalChapters) {
										...
										const percentage =
											data.data.percentage ||
											Math.round((chapterNum / totalChapters) * 100);
								...
								case "scene_content_progress":
									if (data.data?.currentScene && data.data?.totalScenes) {
										...
										const percentage =
											data.data.percentage ||
											Math.round((sceneNum / totalScenes) * 100);
		...
		<Card>
			<CardHeader>
				...
				<p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
					Powered by the Adversity-Triumph Engine - Create emotionally resonant
					stories with deep character development and moral frameworks
				</p>
			</CardHeader>
			<CardContent>
				<form onSubmit={handleSubmit} className="space-y-6">
					...
					<div className="space-y-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border">
						...
						<div className="text-sm text-gray-600 dark:text-gray-400 pt-2 border-t">
							...
							{partsCount * chaptersPerPart * scenesPerChapter} scenes
						</div>
					</div>
						...
						<div className="p-4 rounded-md bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
							...
							<div className="flex space-x-2 mt-3">
								<Button
									type="button"
									onClick={() =>
										router.push(`/studio/edit/story/${generatedStoryId}`)
									}
									className="flex-1"
								>
									Open Story
