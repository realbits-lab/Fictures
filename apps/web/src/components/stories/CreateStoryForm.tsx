...
export function CreateStoryForm() {
    ...
    const [characterCount, setCharacterCount] = useState<number>(
        NOVEL_GENERATION_CONSTRAINTS.CHARACTER.DEFAULT,
    );
    const [settingCount, setSettingCount] = useState<number>(
        NOVEL_GENERATION_CONSTRAINTS.SETTING.DEFAULT,
    );
    const [partsCount, setPartsCount] = useState<number>(
        NOVEL_GENERATION_CONSTRAINTS.PARTS.DEFAULT,
    );
    const [chaptersPerPart, setChaptersPerPart] = useState<number>(
        NOVEL_GENERATION_CONSTRAINTS.CHAPTERS_PER_PART.DEFAULT,
    );
    const [scenesPerChapter, setScenesPerChapter] = useState<number>(
        NOVEL_GENERATION_CONSTRAINTS.SCENES_PER_CHAPTER.DEFAULT,
    );
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState("");
    const [isCompleted, setIsCompleted] = useState(false);
    const [generatedStoryId, setGeneratedStoryId] = useState<string | null>(
        null,
    );
    ...
    const getDefaultProgressSteps = (): ProgressStep[] => [
        ...
        {
            phase: "Characters",
            summary: "Expanding character profiles with detailed arcs",
            status: "pending",
        },
        {
            phase: "Settings",
            summary: "Creating immersive locations with adversity elements",
            status: "pending",
        },
        {
            phase: "Parts",
            summary: "Structuring three-act framework with macro arcs",
            status: "pending",
        },
        {
            phase: "Chapters",
            summary: "Generating detailed chapter structure",
            status: "pending",
        },
        {
            phase: "Scene Summaries",
            summary: "Breaking down chapters into scene outlines",
            status: "pending",
        },
        {
            phase: "Scene Content",
            summary: "Generating narrative content for each scene",
            status: "pending",
        },
        {
            phase: "Scene Evaluation",
            summary: "Evaluating and improving scene quality",
            status: "pending",
        },
        {
            phase: "Images",
            summary: "Creating AI images for characters and settings",
            status: "pending",
        },
    ];
                ...
                for (const line of lines) {
                    if (line.startsWith("data: ")) {
                        try {
                            ...
                            console.log(
                                `⏱️ [${new Date().toISOString()}] Frontend received SSE event: ${data.phase}`,
                            );
                            ...
                            switch (data.phase) {
                                ...
                                case "characters_progress":
                                    console.log(
                                        "[Frontend] Received characters_progress:",
                                        data.data,
                                    );
                                    if (
                                        data.data?.currentCharacter !==
                                            undefined &&
                                        ...
                                        console.log(
                                            `[Frontend] Updating characters progress: ${characterNum} of ${totalCharacters} (${percentage}%)`,
                                        );
                                        setProgress((prev) =>
                                            prev.map((step, index) =>
                                                index === 1
                                ...
                                case "settings_progress":
                                    console.log(
                                        "[Frontend] Received settings_progress:",
                                        data.data,
                                    );
                                    if (
                                        data.data?.currentSetting !==
                                            undefined &&
                                        data.data?.totalSettings
                                    ) {
                                        const settingNum =
                                            data.data.currentSetting;
                                        const totalSettings =
                                        ...
                                        const percentage =
                                            data.data.percentage ||
                                            Math.round(
                                                (settingNum / totalSettings) *
                                                    100,
                                            );

                                        console.log(
                                            `[Frontend] Updating settings progress: ${settingNum} of ${totalSettings} (${percentage}%)`,
                                        );
                                        setProgress((prev) =>
                                            prev.map((step, index) =>
                                                index === 2
                                ...
                                case "parts_progress":
                                    console.log(
                                        "[Frontend] Received parts_progress:",
                                        data.data,
                                    );
                                    if (
                                        data.data?.currentPart !== undefined &&
                                        data.data?.totalParts
                                    ) {
                                        ...
                                        const percentage =
                                            data.data.percentage ||
                                            Math.round(
                                                (partNum / totalParts) * 100,
                                            );

                                        console.log(
                                            `[Frontend] Updating parts progress: ${partNum} of ${totalParts} (${percentage}%)`,
                                        );
                                        setProgress((prev) =>
                                            prev.map((step, index) =>
                                                index === 3
                                ...
                                case "scene_summaries_complete":
                                    ...
                                    if (data.data?.sceneSummaries) {
                                        setStoryData((prev) => ({
                                            ...prev,
                                            scenes: data.data.sceneSummaries,
                                        }));
                                        // @ts-expect-error
                                        setJsonData((prev) => ({
                                            ...prev,
                                            scenesJson: JSON.stringify(
                                                data.data.sceneSummaries,
                                                null,
                                                2,
                                            ),
                                        }));
                                ...
                                case "scene_content_progress":
                                    if (
                                        data.data?.currentScene &&
                                        data.data?.totalScenes
                                    ) {
                                ...
                                case "complete": {
                                    ...
                                    const completedStoryId =
                                        data.data?.storyId || data.storyId;
                                    console.log(
        ...
        <Card>
            <CardHeader>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
                    Powered by the Adversity-Triumph Engine - Create emotionally
                    resonant stories with deep character development and moral
                    frameworks
                </p>
            </CardHeader>
            <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="space-y-2 col-span-2">
                                <Label htmlFor="scenesPerChapter">
                        ...
                        <div className="text-sm text-gray-600 dark:text-gray-400 pt-2 border-t">
                            Total: {partsCount} part{partsCount > 1 ? "s" : ""},{" "}
                            {partsCount * chaptersPerPart} chapter
                            {partsCount * chaptersPerPart > 1 ? "s" : ""}, ~
                            {partsCount * chaptersPerPart * scenesPerChapter}{" "}
                            scenes
                        </div>
                    </div>
                        ...
                        <div className="p-4 rounded-md bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
                            ...
                            <div className="flex space-x-2 mt-3">
                                <Button
                                    ...
                                    onClick={() =>
                                        router.push(
                                            `/studio/edit/story/${generatedStoryId}`,
                                        )
                                    }
                                    className="flex-1"
                                >
                    ...
                    <div className="flex space-x-4">
                        ...
                        <Button
                            type="submit"
                            disabled={
                                isLoading || !prompt.trim() || isCompleted
                            }
                            className="flex-1"
                        >
                            {isLoading
