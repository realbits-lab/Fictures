		...
		stories.length > 0
			? {
					...
					totalPosts: stories.reduce(
						(sum: number, story: CommunityStory) => sum + story.totalPosts,
						0,
					),
					totalMembers: stories.reduce(
						(sum: number, story: CommunityStory) => sum + story.totalMembers,
						0,
					),
					activeToday: stories.filter((story: CommunityStory) => story.isActive)
						.length,
				...
				{stories.length > 0 ? (
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
						{stories.map((story: CommunityStory, index: number) => {
							const isNewlyPublished = recentlyPublishedStories.includes(
								story.id,
							);
							return (
								<div
					...
					<Card>
						<CardContent className="py-12 text-center">
							...
							<p className="text-gray-600 dark:text-gray-400 mb-4">
								Stories will appear here once authors make them public for
