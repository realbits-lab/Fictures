...
function CommunityPost({
	...
	const { executeAction: handleReply, requiresAuth: replyRequiresAuth } =
		useProtectedAction(() => {
			setShowReplies(true);
		});
	...
	return (
		<Card
			className={`${post.isPinned ? "border-yellow-300 dark:border-yellow-600" : ""}`}
		>
						...
						<div className="flex-1">
							...
							<div className="text-xs text-gray-500 dark:text-gray-400">
								...
								{post.lastActivity !== post.createdAt && (
									<span>
										{" "}
										• Last activity {formatRelativeTime(post.lastActivity)}
									</span>
								)}
							</div>
						</div>
					...
					<div className="flex items-center gap-2">
						...
						{session && !isAuthor && (
							<Button
								...
								className="text-xs text-gray-400 hover:text-gray-600"
								onClick={() =>
									toast.info("Reporting functionality would be here")
								}
							>
								⚠️ Report
							</Button>
