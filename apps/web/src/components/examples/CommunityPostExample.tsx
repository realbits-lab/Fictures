...
import { useProtectedAction, useRoleProtectedAction } from '@/hooks/useProtectedAction';
...
export function CommunityPostExample({ 
  ...
  const { executeAction: handleComment, requiresAuth: commentRequiresAuth } = useProtectedAction(() => {
  ...
  const { executeAction: handleCreatePost, isAuthorized: canCreatePost } = useRoleProtectedAction(() => {
    console.log('Creating new post');
  }, ['writer', 'manager'], '/write/new');
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-4">
      ...
      <div className="flex items-center space-x-4 border-t border-gray-200 dark:border-gray-700 pt-4">
        ...
        >
          <span>💬</span>
          <span>{comments}</span>
          {commentRequiresAuth && <span className="text-xs">(sign in)</span>}
        </Button>
      ...
      <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-700 rounded text-xs">
        ...
        {session ? (
          <span className="text-green-600 dark:text-green-400">
            Signed in as {session.user?.name} ({session.user?.role || 'reader'})
          </span>
        ) : (
          <span className="text-orange-600 dark:text-orange-400">
            Not signed in - can browse but interactions require authentication
          </span>
        )}
      </div>
    </div>
