...
export function UnifiedWritingEditor({
	story: initialStory,
	allStories,
	initialSelection,
	disabled = false,
	...
	useEffect(() => {
		...
		return () => {
			const result = writingSession.endSession() as {
				duration: number;
				wordsWritten: number;
			} | null;
			if (result) {
				console.log(
					`✅ Writing session ended: ${result.duration}ms, ${result.wordsWritten} words written`,
				);
			}
		};
	...
	const parseStoryData = (): StoryDocument => {
		...
		const hnsStory: any = {
			...
			genre: (Array.isArray(story.genre)
				? story.genre
				: [story.genre || parsedData?.genre || "General"]) as any,
			premise: parsedData?.premise || "Story premise",
			dramatic_question:
				parsedData?.question ||
				parsedData?.dramatic_question ||
			...
			structural_role:
				index === 0
					...
					: index === 1
						...
						: "Act 3: Resolution",
			summary:
				parsedData?.parts?.[index]?.summary || `Summary for ${part.title}`,
		...
		story.parts.forEach((part, partIndex) => {
			part.chapters.forEach((chapter, chapterIndex) => {
				hnsChapters.push({
					...
					part_ref: part.id,
					summary:
						parsedData?.parts?.[partIndex]?.chapters?.[chapterIndex]?.summary ||
						"",
					pacing_goal: "medium",
					...
					chapter_hook: {
						type: "question",
						summary:
							parsedData?.parts?.[partIndex]?.chapters?.[chapterIndex]?.hook ||
							"",
	...
	const [sampleStoryData, setSampleStoryData] = useState<StoryDocument>(
		parseStoryData(),
	);
	...
	const handleSelectionChange = async (selection: Selection) => {
		...
		if (
			...
			if (
				selection.chapterId !== currentSelection.chapterId ||
		...
		try {
			...
			if (
				...
				if (!response.ok) {
					...
					throw new Error(
						`Failed to save scene: ${errorData.error || response.statusText}`,
					);
				}
				...
				if (!response.ok) {
					...
					throw new Error(
						`Failed to save chapter: ${errorData.error || response.statusText}`,
					);
				}
				...
				if (!response.ok) {
					const errorData = await response.json();
					throw new Error(
						`Failed to save part: ${errorData.error || response.statusText}`,
					);
				}
	...
	const handlePublishToggle = async () => {
		...
		for (const part of story.parts) {
			const foundChapter = part.chapters.find(
		...
		if (!currentChapter) {
			const foundChapter = story.chapters.find(
				(ch) => ch.id === currentSelection.chapterId,
			);
			if (foundChapter) {
				currentChapter = foundChapter;
		...
		if (currentChapter && currentChapterStatus === "published") {
			...
			const scenesWithContent =
				currentChapter.scenes?.filter(
					(s) => (s as any).content && (s as any).content.trim() !== "",
				) || [];
		...
		if (!isPublished) {
			...
			if (chapterScenes.length === 0) {
				alert(
					"Cannot publish chapter without scenes. Please add at least one scene before publishing.",
				);
				return;
			}
			const scenesWithContent = chapterScenes.filter(
				(s) => (s as any).content && (s as any).content.trim() !== "",
			);
			if (scenesWithContent.length === 0) {
				alert(
					"Cannot publish chapter with empty scenes. Please add content to at least one scene before publishing.",
				);
				return;
			}
		}
	...
	const handleToggleSceneStatus = async (
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					`Failed to update scene status: ${errorData.error || "Unknown error"}`,
				);
			}
	...
	const createPartData = (partNum: number, partTitle: string) => {
		...
		const currentPart = sampleStoryData.parts.find(
			(p: any) => p.part_title === partTitle || p.part_id === `part_${partNum}`,
		);
		...
		return {
			...
			goal:
				currentPart?.summary ||
				`Part ${partNum} goal from ${sampleStoryData.story.story_title}`,
	...
	const findSceneData = () => {
		...
		const createSceneData = (scene: any, sceneNum: number) => ({
			...
			time:
				sceneNum === 1 ? "morning" : sceneNum === 2 ? "afternoon" : "evening",
			place: scene?.title?.toLowerCase().includes("apartment")
				? "elena_apartment"
				: scene?.title?.toLowerCase().includes("office")
					? "maya_office"
					: "location",
			pov: "maya",
			characters: {
				...
				elena: {
					status: scene?.id?.includes("elena") ? "present" : "referenced",
				},
			},
	...
	const renderEditor = () => {
		switch (currentSelection.level) {
									...
									<table className="w-full text-sm">
										<tbody>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800 w-1/3">
													ID
												</td>
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(story as any).rating
														? ((story as any).rating / 10).toFixed(1)
														: "N/A"}
												</td>
											</tr>
											<tr className="border-b">
			...
			case "part": {
				const selectedPart = story.parts.find(
					(p) => p.id === currentSelection.partId,
				);
				if (!selectedPart) {
					return (
						<Card>
									...
									<table className="w-full text-sm">
										<tbody>
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4 font-mono text-xs">
													{(selectedPart as any).storyId || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4 font-mono text-xs">
													{(selectedPart as any).authorId || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedPart as any).summary || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{selectedPart.chapters?.reduce(
														(total, ch) => total + (ch.scenes?.length || 0),
														0,
													)}
												</td>
											</tr>
			...
			case "chapter": {
				...
				// Look in standalone chapters if not found in parts
				if (!selectedChapter) {
					selectedChapter = story.chapters.find(
						(ch) => ch.id === currentSelection.chapterId,
					);
				}
				...
				const createChapterData = (chapter: any, partTitle: string | null) => {
					...
					if (
						...
						console.warn(
							`Chapter ${chapter.id} (${chapter.title}) is marked as published but has no content. Showing as draft.`,
						);
						actualStatus = "draft";
					}
				...
				if (!chapterData) {
					return (
						<ContentLoadError
							...
							icon="chapter"
							onRetry={() =>
								handleSelectionChange({ level: "story", storyId: story.id })
							}
							compact={false}
						/>
					);
									...
									<table className="w-full text-sm">
										<tbody>
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).summary || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4 font-mono text-xs">
													{(selectedChapter as any).storyId || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4 font-mono text-xs">
													{(selectedChapter as any).partId || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4 font-mono text-xs">
													{(selectedChapter as any).authorId || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).purpose || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).hook || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).characterFocus || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).publishedAt
														? new Date(
																(selectedChapter as any).publishedAt,
															).toLocaleString()
														: "N/A"}
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).scheduledFor
														? new Date(
																(selectedChapter as any).scheduledFor,
															).toLocaleString()
														: "N/A"}
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4 font-mono text-xs">
													{(selectedChapter as any).characterId || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).arcPosition || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).contributesToMacroArc ||
														"N/A"}
												</td>
											</tr>
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).focusCharacters &&
													(selectedChapter as any).focusCharacters.length >
														0 ? (
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).adversityType || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).virtueType || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).seedsPlanted &&
													(selectedChapter as any).seedsPlanted.length > 0 ? (
														<pre className="text-xs bg-gray-100 dark:bg-gray-900 p-2 rounded overflow-auto max-h-60">
															{JSON.stringify(
																(selectedChapter as any).seedsPlanted,
																null,
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).seedsResolved &&
													(selectedChapter as any).seedsResolved.length > 0 ? (
														<pre className="text-xs bg-gray-100 dark:bg-gray-900 p-2 rounded overflow-auto max-h-60">
															{JSON.stringify(
																(selectedChapter as any).seedsResolved,
																null,
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).connectsToPreviousChapter ||
														"N/A"}
												</td>
											</tr>
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).createsNextAdversity ||
														"N/A"}
												</td>
											</tr>
											<tr className="border-b">
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).createdAt
														? new Date(
																(selectedChapter as any).createdAt,
															).toLocaleString()
														: "N/A"}
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedChapter as any).updatedAt
														? new Date(
																(selectedChapter as any).updatedAt,
															).toLocaleString()
														: "N/A"}
			...
			case "scene": {
				...
				for (const part of story.parts) {
					for (const chapter of part.chapters) {
						const foundScene = chapter.scenes?.find(
							(s) => s.id === currentSelection.sceneId,
						);
						if (foundScene) {
									...
									<table className="w-full text-sm">
										<tbody>
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4 font-mono text-xs">
													{(selectedScene as any).chapterId || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{selectedSceneChapter?.title || "Unknown"}
												</td>
											</tr>
											{selectedScenePart && (
												<tr className="border-b">
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).orderIndex}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).imageUrl ? (
														<a
															href={(selectedScene as any).imageUrl}
															target="_blank"
															rel="noopener noreferrer"
															className="text-blue-600 hover:underline text-xs break-all"
														>
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).cyclePhase || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).emotionalBeat || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).dialogueVsDescription ||
														"N/A"}
												</td>
											</tr>
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).suggestedLength || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).publishedAt
														? new Date(
																(selectedScene as any).publishedAt,
															).toLocaleString()
														: "N/A"}
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).scheduledFor
														? new Date(
																(selectedScene as any).scheduledFor,
															).toLocaleString()
														: "N/A"}
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).visibility || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).autoPublish ? "Yes" : "No"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4 font-mono text-xs">
													{(selectedScene as any).publishedBy || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).unpublishedAt
														? new Date(
																(selectedScene as any).unpublishedAt,
															).toLocaleString()
														: "N/A"}
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4 font-mono text-xs">
													{(selectedScene as any).unpublishedBy || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).comicStatus || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).comicPublishedAt
														? new Date(
																(selectedScene as any).comicPublishedAt,
															).toLocaleString()
														: "N/A"}
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4 font-mono text-xs">
													{(selectedScene as any).comicPublishedBy || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
													Comic Unpublished At
												</td>
												<td className="py-2 px-4">
													{(selectedScene as any).comicUnpublishedAt
														? new Date(
																(selectedScene as any).comicUnpublishedAt,
															).toLocaleString()
														: "N/A"}
											...
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
													Comic Unpublished By
												</td>
												<td className="py-2 px-4 font-mono text-xs">
													{(selectedScene as any).comicUnpublishedBy || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).comicGeneratedAt
														? new Date(
																(selectedScene as any).comicGeneratedAt,
															).toLocaleString()
														: "N/A"}
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).comicPanelCount || 0}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).comicVersion || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).viewCount || 0}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).uniqueViewCount || 0}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).novelViewCount || 0}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
													Novel Unique View Count
												</td>
												<td className="py-2 px-4">
													{(selectedScene as any).novelUniqueViewCount || 0}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).comicViewCount || 0}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
													Comic Unique View Count
												</td>
												<td className="py-2 px-4">
													{(selectedScene as any).comicUniqueViewCount || 0}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).lastViewedAt
														? new Date(
																(selectedScene as any).lastViewedAt,
															).toLocaleString()
														: "N/A"}
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).createdAt
														? new Date(
																(selectedScene as any).createdAt,
															).toLocaleString()
														: "N/A"}
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{(selectedScene as any).updatedAt
														? new Date(
																(selectedScene as any).updatedAt,
															).toLocaleString()
														: "N/A"}
			...
			case "characters": {
				...
				const selectedCharacter = (story as any).characters?.find(
					(c: any) => c.id === currentSelection.characterId,
				);
									...
									<table className="w-full text-sm">
										<tbody>
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{selectedCharacter.isMain ? "Yes" : "No"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{selectedCharacter.imageUrl ? (
														<a
															href={selectedCharacter.imageUrl}
															target="_blank"
															rel="noopener noreferrer"
															className="text-blue-600 hover:underline text-xs break-all"
														>
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{selectedCharacter.archetype || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{selectedCharacter.storyline || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4 font-mono text-xs">
													{selectedCharacter.visualReferenceId || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{selectedCharacter.coreTrait || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{selectedCharacter.internalFlaw || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
												...
												<td className="py-2 px-4">
													{selectedCharacter.externalGoal || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
			...
			case "settings": {
				...
				const selectedSetting = (story as any).settings?.find(
					(s: any) => s.id === currentSelection.settingId,
				);
				...
				return (
					<div className="space-y-6">
						<Card>
							<CardHeader>
								<CardTitle>
									🗺️ Setting Details - {selectedSetting.name}
								</CardTitle>
							</CardHeader>
							<CardContent>
								<div className="overflow-x-auto">
									<table className="w-full text-sm">
										<tbody>
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{selectedSetting.description || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{selectedSetting.architecturalStyle || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{selectedSetting.symbolicMeaning || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
											...
											<tr className="border-b">
												...
												<td className="py-2 px-4">
													{selectedSetting.emotionalResonance || "N/A"}
												</td>
											</tr>
											<tr className="border-b">
												<td className="py-2 px-4 font-medium bg-gray-50 dark:bg-gray-800">
							...
							<div className="flex items-center gap-1 md:gap-3">
								{currentSelection.level === "story" && (
									<Button
										...
										className={
											story.status === "published"
												? "bg-[rgb(var(--color-primary))] hover:bg-[rgb(var(--color-primary)/90%)] text-[rgb(var(--color-primary-foreground))]"
												: "bg-[rgb(var(--color-muted))] hover:bg-[rgb(var(--color-muted)/80%)] text-[rgb(var(--color-muted-foreground))]"
										}
										title={
											story.status === "published"
						...
						>
							...
							>
								<StoryStructureSidebar
									...
									validatingStoryId={isValidatingCurrentStory ? story.id : null}
								/>
							</div>
						</Panel>
