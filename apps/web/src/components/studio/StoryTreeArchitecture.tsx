		...
		if (currentSelection) {
			...
			if (currentSelection.level === "scene" && currentSelection.chapterId) {
				...
				for (const part of story.parts) {
					if (
						part.chapters.some((ch) => ch.id === currentSelection.chapterId)
					) {
										...
										>
											<div className="flex items-center gap-2">
												<span className="text-sm font-medium text-green-600 dark:text-green-400 truncate">
													{getStatusIcon(part.status || "draft")} Part{" "}
										...
										{part.chapters.map((chapter, chapterIndex) => {
											...
											const globalChapterNumber =
												story.parts
													.slice(
														0,
														story.parts.findIndex((p) => p.id === part.id),
													)
													.reduce(
														(total, prevPart) =>
															total + (prevPart.chapters?.length || 0),
														0,
													) +
												chapterIndex +
												1;
												...
												>
													<div className="flex items-center justify-between mb-1">
														<div className="flex items-center gap-1 flex-1">
															...
															<button
																onClick={() =>
																	handleChapterClick(chapter.id, part.id)
																}
						...
						{story.chapters.length > 0 && (
							<div className="border-l-2 border-orange-400 pl-3">
								...
								{story.chapters.map((chapter, chapterIndex) => {
									...
									const totalPartsChapters = story.parts.reduce(
												...
												<div className="space-y-1 mt-2 ml-4">
													{chapter.scenes.map((scene, sceneIndex) => (
														...
														>
															<button
																onClick={() =>
																	handleSceneClick(scene.id, chapter.id)
																}
																className={`w-full flex items-center gap-2 p-2 rounded text-xs transition-colors ${
																	isSelectedLevel("scene", scene.id)
																		? "bg-purple-100 text-purple-900 dark:bg-purple-900/20 dark:text-purple-100"
