...
export function ChapterEditor({
    ...
    const [content, setContent] =
        useState(`  The Shadow Realm pulsed around Maya like a living thing, its twisted architecture bending reality with each heartbeat. She could feel Elena's presence—faint but unmistakable—calling to her from the void-touched spire ahead.
  ...
  Maya's shadows writhed, responding to her emotional turmoil. Part of her—the part she'd been fighting since this began—whispered that he was right. Why should she hold back? Elena was dying. The world was at stake.`);
    ...
    useEffect(() => {
        ...
        if (currentWordCount > 5000 * 1.5) {
            // Default target is 5000 words
    ...
    const sampleStoryData = {
        ...
        hooks: {
            overarching: [
                "elena_fate",
                "maya_corruption_risk",
                "shadow_magic_truth",
            ],
            mysteries: [
                "previous_student_identity",
                "marcus_secret",
                "realm_connection",
            ],
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1 md:gap-3">
                                <span className="text-xs md:text-sm text-gray-600 dark:text-gray-400 hidden sm:block">
                                    {isAutoSaving
                                        ? "💾 Saving..."
                                        : `💾 ${hasUnsavedChanges ? "Unsaved changes" : `Saved ${formatLastSaved(lastSaved)}`}`}
                                </span>
                                <Button
                                    variant="ghost"
                                    size="sm"
            ...
            <div className="container mx-auto px-4 py-6">
                <div
                    className={`grid gap-6 ${hideSidebar ? "grid-cols-1" : "grid-cols-1 lg:grid-cols-4"}`}
                >
                    ...
                    {/* Main Writing Area */}
                    <div
                        className={`space-y-6 ${hideSidebar ? "col-span-1" : "lg:col-span-2"}`}
                    >
