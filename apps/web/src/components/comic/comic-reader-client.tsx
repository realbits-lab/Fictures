...
export function ComicReaderClient({
	storyId,
	initialData,
	...
	// Fetch story data
	const { story, availableChapters, isLoading, error } = useStoryReader(
		storyId,
		initialData,
	);
			...
			<div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-6">
				<div className="max-w-lg w-full text-center">
					...
					<p className="text-gray-600 dark:text-gray-400 mb-8 leading-relaxed text-lg">
						We&apos;re having trouble loading this story. Don&apos;t worry,
						it&apos;s still here! Try refreshing the page or come back in a few
						moments.
					</p>
												...
												<div className="flex-1 min-w-0">
													...
													{item.scene.comicUniqueViewCount !== undefined && (
														<div className="flex items-center gap-1 mt-1 text-xs text-gray-400 dark:text-gray-500">
															<span>👁️</span>
															<span>
																{item.scene.comicUniqueViewCount.toLocaleString()}
															</span>
														</div>
													)}
												</div>
				...
				<div className="flex-1 overflow-y-auto bg-white">
					{selectedScene ? (
						<div className="w-full max-w-md md:max-w-4xl mx-auto pb-24 md:pb-8">
							{/* Comic Viewer */}
							<ComicViewer sceneId={selectedScene.id} className="" />
						</div>
					) : (
