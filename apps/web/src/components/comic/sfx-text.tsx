...
export function SFXText({
	...
	return (
		<div
			className={cn("absolute z-20 pointer-events-none select-none", className)}
			style={{
				left: `${position.x}%`,
				top: `${position.y}%`,
...
export function SFXTextGroup({ sfxList, className }: SFXTextGroupProps) {
	...
	return (
		<div className={className}>
			{sfxList.map((sfx, index) => {
				const { position, rotation } = getAutoPosition(
					index,
					sfxList.length,
					sfx.emphasis,
				);
