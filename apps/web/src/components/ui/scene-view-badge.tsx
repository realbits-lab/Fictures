...
export function SceneViewBadge({
  ...
  if (mode === 'split' && novelViews !== undefined && comicViews !== undefined) {
    return (
  ...
  if (mode === 'detailed') {
    return (
      <div className={`flex flex-col gap-1 ${className}`}>
        <div className={`inline-flex items-center gap-1.5 text-gray-600 dark:text-gray-400 ${sizeClasses[size]}`}>
          <Eye className={iconSizeClasses[size]} aria-hidden="true" />
        ...
        {novelViews !== undefined && comicViews !== undefined && (
          <div className="flex items-center gap-2 text-xs">
            <span className="text-blue-600 dark:text-blue-400">📖 {formatCount(novelViews)}</span>
            <span className="text-purple-600 dark:text-purple-400">🎨 {formatCount(comicViews)}</span>
          </div>
        )}
      </div>
    );
