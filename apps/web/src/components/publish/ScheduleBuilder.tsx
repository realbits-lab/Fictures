...
export function ScheduleBuilder({
	...
	const calculateEstimate = () => {
		...
		const totalPublications = Math.min(
			publications,
			Math.ceil(totalScenes / scenesPerPublish),
		);
		...
		<form onSubmit={handleSubmit} className="space-y-6">
			<div>
				...
				<p className="text-gray-600 dark:text-gray-400">
					Automate your publishing workflow with a custom schedule for{" "}
					{totalScenes} scenes.
				</p>
			</div>
				...
				<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
					{[
						...
						<button
							...
							type="button"
							onClick={() =>
								setFormData({ ...formData, scheduleType: type.value as any })
							}
							className={cn(
			...
			<div>
				...
				<input
					...
					onChange={(e) => {
						setFormData({
							...formData,
							scenesPerPublish: parseInt(e.target.value),
						});
						calculateEstimate();
					}}
					className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
			...
			{estimatedPublications > 0 && (
				<div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
					<p className="text-sm text-blue-800 dark:text-blue-200">
						<strong>Estimated:</strong> {estimatedPublications} publications
						will be scheduled
						{formData.endDate &&
