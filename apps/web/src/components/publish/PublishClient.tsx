...
export function PublishClient() {
	...
	if (hasError) {
		const errorMessage =
			statusError?.message || historyError?.message || analyticsError?.message;
		return (
						...
						<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
							...
							<div className="flex items-center gap-2">
								...
								<Badge variant="default">
									{publishStatus?.readyToPublish?.shortTitle ||
										"Final Confrontation"}{" "}
									✅
								</Badge>
							</div>
								...
								<div className="flex gap-4">
									...
									<label className="flex items-center gap-2">
										...
										<span className="text-sm">
											🔘 Schedule:{" "}
											{publishStatus?.readyToPublish?.scheduledTime ||
												"Nov 15, 2:00 PM"}
										</span>
									</label>
									<label className="flex items-center gap-2">
							...
							<div>
								...
								<div className="space-y-2">
									...
									<label className="flex items-center gap-2">
										...
										<span className="text-sm">
											☑️ Community poll:{" "}
											{publishStatus?.readyToPublish?.communityPoll ||
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
				<Card>
					...
					<CardContent className="space-y-4">
						<div>
							<h4 className="font-medium text-gray-900 dark:text-gray-100 mb-3">
								{publishAnalytics?.latestChapter?.title || "Chapter 15"}{" "}
								Performance
							</h4>
							<div className="text-sm text-gray-600 dark:text-gray-400 mb-3">
								Published:{" "}
								{publishAnalytics?.latestChapter?.publishedAgo || "3 days ago"}
							</div>
							...
							<div className="grid grid-cols-2 gap-4 text-sm">
								...
								<div>
									⭐ Rating:{" "}
									<span className="font-medium">
										{publishAnalytics?.latestChapter?.rating || "4.9"}/5
									</span>
								</div>
							</div>
							...
							<div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
								<div className="flex justify-between text-sm mb-1">
									...
									<span className="font-medium">
										{publishAnalytics?.latestChapter?.engagementRate || "87"}%
									</span>
								</div>
								<div className="text-sm text-green-600">
									🔥 Trending: #
									{publishAnalytics?.latestChapter?.trendingRank || "2"} in{" "}
									{publishAnalytics?.latestChapter?.genre || "Fantasy"}
								</div>
							</div>
						</div>
					</CardContent>
					...
					<CardContent className="space-y-4">
						<div>
							...
							<div className="space-y-2 text-sm">
								<div className="flex justify-between">
									...
									<span className="font-medium text-green-600">
										+{publishAnalytics?.prepublishBuzz?.theories || "89"} new
									</span>
								</div>
								<div className="flex justify-between">
									<span className="text-gray-600 dark:text-gray-400">
									...
									<span className="font-medium text-green-600">
										+{publishAnalytics?.prepublishBuzz?.comments || "234"}
									</span>
								</div>
								<div className="flex justify-between">
									<span className="text-gray-600 dark:text-gray-400">
									...
									<span className="font-medium">
										{publishAnalytics?.prepublishBuzz?.anticipation || "94"}%
									</span>
								</div>
							</div>
							...
							<div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
								...
								<div className="text-sm text-gray-600 dark:text-gray-400">
									{publishAnalytics?.optimalTime?.time || "Wed 2:00 PM PST"}
									<br />
									<span className="text-xs">
										(
										{publishAnalytics?.optimalTime?.activeReaderPercentage ||
											"89"}
										% readers active)
									</span>
								</div>
