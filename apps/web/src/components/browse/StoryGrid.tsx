					...
					<div className="flex items-center justify-between md:justify-end gap-3">
						...
						<div className="inline-flex rounded-lg border border-[rgb(var(--color-border))] bg-[rgb(var(--color-background))] flex-1 md:flex-initial">
							<Select
								value={sortBy}
								onValueChange={(value) =>
									setSortBy(value as "latest" | "popular" | "rating")
								}
							>
								<SelectTrigger className="border-0 bg-transparent hover:bg-[rgb(var(--color-muted))] focus:ring-0 focus:ring-offset-0 w-full">
				...
				viewMode === "card" ? (
					...
					>
						{sortedStories.map((story, index) => {
							...
							const shouldShowAd =
								(index + 1) % 8 === 0 && index !== sortedStories.length - 1;
							...
							return (
								<React.Fragment key={story.id}>
									...
									<div
										data-testid={
											pageType === "community" ? "post-card" : "story-card"
										}
										key={story.id}
										onClick={async () => {
									...
									>
										...
										<div className="p-4 flex flex-col flex-grow">
											...
											<div
												data-testid={
													pageType === "community" ? "post-author" : undefined
												}
