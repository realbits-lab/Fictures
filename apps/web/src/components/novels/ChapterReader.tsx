...
export function ChapterReader({ story, isOwner }: ChapterReaderProps) {
	...
	useEffect(() => {
		if (!selectedChapterId && availableChapters.length > 0) {
			const firstPublishedChapter =
				availableChapters.find((ch) => ch.status === "published") ||
				availableChapters[0];
	...
	return (
		<div
			...
			className="absolute inset-0 top-16 bg-white dark:bg-gray-900 flex flex-col"
		>
				...
				<div className="flex items-center justify-between px-4 md:px-6 py-3">
							...
							<div className="hidden lg:flex items-center gap-2">
								...
								<div className="flex items-center gap-2">
									...
									<span className="text-sm font-medium text-gray-700 dark:text-gray-300 truncate max-w-sm">
										Ch {getGlobalChapterNumber(selectedChapter.id)}:{" "}
					...
					<div className="flex items-center gap-2 md:gap-3">
						...
						{(() => {
							...
							const prevChapter =
								currentIndex > 0 ? availableChapters[currentIndex - 1] : null;
							const nextChapter =
							...
							return (
								<>
									<button
										onClick={() =>
											prevChapter && setSelectedChapterId(prevChapter.id)
										}
									...
									<button
										onClick={() =>
											nextChapter && setSelectedChapterId(nextChapter.id)
										}
					...
					>
						<div className="p-4">
							...
							{story.parts.map((part) => {
								const partChapters = part.chapters.filter(
									(chapter) => isOwner || chapter.status === "published",
								);
								...
								return (
									<div key={part.id} className="mb-4">
										...
										{partChapters.map((chapter) => {
											const globalChapterNumber = getGlobalChapterNumber(
												chapter.id,
											);
						...
						<article className="max-w-4xl mx-auto px-4 md:px-8 py-6 md:py-8">
							...
							<header className="mb-6 md:mb-8 border-b border-gray-200 dark:border-gray-800 pb-4 md:pb-6">
								<div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-3">
									...
									<span>
										Chapter {getGlobalChapterNumber(selectedChapter.id)}
									</span>
