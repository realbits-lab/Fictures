...
export function ChapterReaderClient({
    ...
    console.log(
        `[${componentId.current}] 🎭 ChapterReaderClient MOUNT - Story: ${storyId}${initialData ? " (with SSR data)" : " (client-side only)"}`,
    );
    ...
    const scrollPositionKey = React.useCallback(
        (sceneId: string) => `fictures_scene_scroll_${storyId}_${sceneId}`,
        [storyId],
    );
            ...
            scrollSaveTimeoutRef.current = setTimeout(() => {
                try {
                    ...
                    if (!Number.isNaN(position)) {
                        ...
                        console.log(
                            `💾 Saved scroll position for scene ${sceneId}: ${position}px`,
                        );
                    }
                } catch (error) {
                    ...
                    if (
                        ...
                        console.log(
                            "Attempting to clear old scroll positions due to quota...",
                        );
    ...
    const getScrollPosition = React.useCallback(
        (sceneId: string): number => {
            try {
                ...
                console.log(
                    `📖 Retrieved scroll position for scene ${sceneId}: ${position}px`,
                );
                return position;
            } catch (error) {
    ...
    const handleSceneSelect = React.useCallback(
        (sceneId: string, chapterId: string) => {
            ...
            console.log(
                `[${navId}] 🧭 NAVIGATION START - Scene: ${sceneId}, Chapter: ${chapterId}`,
            );
            ...
            if (selectedSceneId && mainContentRef.current) {
                ...
                console.log(
                    `[${navId}] 💾 Saved scroll position (${currentScrollTop}px) in ${saveDuration.toFixed(2)}ms`,
                );
            }
            ...
            const stateUpdateDuration =
                performance.now() - stateUpdateStartTime;
            console.log(
                `[${navId}] 🔄 State updates: ${stateUpdateDuration.toFixed(2)}ms`,
            );
            ...
            const currentIndex = allScenes.findIndex(
                (item) => item.scene.id === sceneId,
            );
            if (currentIndex >= 0) {
                const prevScene =
                    currentIndex > 0 ? allScenes[currentIndex - 1] : null;
                const nextScene =
                    currentIndex < allScenes.length - 1
                        ? allScenes[currentIndex + 1]
                        : null;

                console.log(
                    `[${navId}] 🔮 Triggering prefetch - Prev: ${prevScene?.chapterId || "none"}, Next: ${nextScene?.chapterId || "none"}`,
                );
            ...
            const prefetchTriggerDuration =
                performance.now() - prefetchStartTime;

            const totalNavDuration = performance.now() - navStartTime;
            console.log(
                `[${navId}] ✅ Navigation sync operations: ${totalNavDuration.toFixed(2)}ms`,
            );
            console.log(
                `[${navId}] 📊 Breakdown: Save=${stateUpdateDuration.toFixed(0)}ms, StateUpdate=${stateUpdateDuration.toFixed(0)}ms, PrefetchTrigger=${prefetchTriggerDuration.toFixed(0)}ms`,
            );
        },
        [
            selectedSceneId,
    ...
    const handleContentTap = React.useCallback((e: React.MouseEvent) => {
        ...
        const isInteractive = target.closest(
            'button, a, input, textarea, select, img, video, [role="button"]',
        );
        if (isInteractive) {
    ...
    useEffect(() => {
        const fetchAllScenes = async () => {
            if (!story || availableChapters.length === 0) {
                console.log(
                    `⚠️ [PARALLEL-FETCH] Skipping parallel fetch: story=${!!story}, availableChapters=${availableChapters.length}`,
                );
                if (story) {
                    console.log(`⚠️ [PARALLEL-FETCH] Story data structure:`);
                    console.log(`⚠️   - parts: ${story.parts?.length ?? 0}`);
                    console.log(
                        `⚠️   - chapters: ${story.chapters?.length ?? 0}`,
                    );
                    if (story.parts?.length > 0) {
                        story.parts.forEach((part: any, idx: number) => {
                            console.log(
                                `⚠️     Part ${idx}: title="${part.title}", chapters=${part.chapters?.length ?? 0}`,
                            );
                        });
                    }
                    if (story.chapters?.length > 0) {
                        story.chapters.forEach((ch: any, idx: number) => {
                            console.log(
                                `⚠️     Chapter ${idx}: title="${ch.title}", status="${ch.status}"`,
                            );
                        });
                    }
                }
            ...
            console.log(
                `🚀 Starting parallel scene fetch for ${availableChapters.length} chapters...`,
            );
            ...
            for (const part of [...story.parts].sort(
            ...
            console.log(
                `🔍 [PARALLEL-FETCH] Built chaptersToFetch array: ${chaptersToFetch.length} chapters`,
            );
            if (chaptersToFetch.length === 0) {
                console.log(
                    `⚠️ [PARALLEL-FETCH] No chapters to fetch! Checking conditions:`,
                );
                console.log(
                    `⚠️   - story.chapters.length: ${story.chapters?.length ?? 0}`,
                );
                console.log(
                    `⚠️   - story.parts.length: ${story.parts?.length ?? 0}`,
                );
            ...
            const fetchPromises = chaptersToFetch.map(async (chapter) => {
                ...
                console.log(
                    `🚀 [PARALLEL] Starting fetch for chapter: ${chapter.title} (${chapter.id})`,
                );
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        console.log(
                            `✅ [PARALLEL] Chapter fetch completed: ${chapter.title} in ${fetchDuration.toFixed(0)}ms (${data.scenes?.length || 0} scenes)`,
                        );
                        return {
                            chapter,
                            scenes: data.scenes || [],
                        ...
                        console.warn(
                            `❌ [PARALLEL] Failed fetch: ${chapter.title} - ${response.status} in ${fetchDuration.toFixed(0)}ms`,
                        );
                        return {
                            chapter,
                            scenes: [],
                    ...
                    const fetchDuration = performance.now() - chapterFetchStart;
                    console.error(
                        `❌ [PARALLEL] Error fetching ${chapter.title} after ${fetchDuration.toFixed(0)}ms:`,
                        error,
                    );
                    return {
                        chapter,
            ...
            console.log(
                `⏳ [PARALLEL] Waiting for ${fetchPromises.length} chapter fetches to complete...`,
            );
            ...
            console.log(
                `✅ Parallel fetch completed in ${duration}ms (${results.length} chapters, ${scenesList.length} scenes)`,
            );
            console.log(
                `⚡ Performance improvement: ${results.length * 500}ms sequential → ${duration}ms parallel = ${((results.length * 500) / parseFloat(duration)).toFixed(1)}x faster`,
            );
        };
    ...
    useEffect(() => {
        if (selectedSceneId && !isScrollRestored) {
            ...
            console.log(
                `[${scrollId}] 📜 SCROLL RESTORE START - Scene: ${selectedSceneId}, Saved position: ${savedPosition}px`,
            );
            ...
            requestAnimationFrame(() => {
                ...
                if (mainContentRef.current && savedPosition > 0) {
                    ...
                    const totalDuration =
                        performance.now() - scrollRestoreStartTime;
                    console.log(
                        `[${scrollId}] ✅ Scroll restored to ${savedPosition}px - RAF: ${scrollSetDuration.toFixed(2)}ms, Total: ${totalDuration.toFixed(2)}ms`,
                    );
                } else {
                    const totalDuration =
                        performance.now() - scrollRestoreStartTime;
                    console.log(
                        `[${scrollId}] ℹ️  No scroll to restore - Total: ${totalDuration.toFixed(2)}ms`,
                    );
                }
    ...
    useEffect(() => {
        ...
        if (!mainContentElement || !selectedSceneId || !isScrollRestored)
            return;
        ...
        mainContentElement.addEventListener("scroll", throttledScroll, {
            passive: true,
        });
        return () => {
    ...
    useEffect(() => {
        ...
        if (sidebarElement) {
            sidebarElement.addEventListener("wheel", handleSidebarWheel, {
                passive: false,
            });
        }
    ...
    return (
        <div
            ...
            className={`absolute inset-0 ${isUIVisible ? "top-16 z-auto" : "top-0 z-[70]"} bg-white dark:bg-gray-900 flex flex-col transition-[top] duration-300 ease-in-out`}
        >
            {/* Second GNB - Reading Navigation Header */}
            <div
                className={`${isUIVisible ? "sticky" : "fixed"} top-0 left-0 right-0 z-40 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 transition-transform duration-300 ease-in-out ${
        ...
        `}
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        {item.scene
                                                            ...
                                                            <div className="flex items-center gap-1 mt-1 text-xs text-gray-400 dark:text-gray-500">
                                                                <span>👁️</span>
                                                                <span>
                                                                    {item.scene.novelUniqueViewCount.toLocaleString()}
                                                                </span>
                                                            </div>
                                                        )}
                                                    </div>
                ...
                >
                    {selectedChapter ? (
                        <article className="max-w-4xl mx-auto px-8 py-8 pb-24 md:pb-8">
                            {/* Scene Content */}
                            <div className="prose prose-lg max-w-none text-gray-900 dark:text-gray-100">
                                {scenesError ? (
                                    <div className="text-center py-12 text-red-500 dark:text-red-400">
                                        <div className="max-w-md mx-auto">
                                            ...
                                            <p className="text-sm mb-4">
                                                Failed to load scenes for this
                                                chapter.
                                            </p>
                                            <button
                                                onClick={() =>
                                  ...
                                  chapterScenes.length === 0 ? (
                                    ...
                                    <>
                                        {console.log(
                                            `📖 Rendering selected scene: ${selectedScene.title}`,
                                        )}
                                    ...
                                    <>
                                        {console.log(
                                            `⚠️  Chapter has no scenes: ${selectedChapter?.title} - Architecture violation!`,
                                        )}
                                            ...
                                            <div className="mt-12 text-center">
                                                <div className="inline-flex items-center gap-3 px-6 py-3 bg-gray-100 dark:bg-gray-800 rounded-full">
                                                    ...
                                                    <span className="text-sm text-gray-600 dark:text-gray-400">
                                                        {isOwner
                                                            ? "Create scenes to publish this chapter"
