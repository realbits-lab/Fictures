...
export async function improveStoryContent(
    ...
    console.log(
        "     - Validation Errors:",
        analysisResult.validation?.totalErrors || 0,
    );
    console.log(
        "     - Validation Warnings:",
        analysisResult.validation?.totalWarnings || 0,
    );
    console.log(
        "     - Overall Score:",
        analysisResult.evaluation?.storyEvaluation?.overallScore || 0,
    );
    ...
    if (
        ...
        console.log(
            "   Story improvement completed in",
    ...
    console.log(
        "   Total parts to process:",
        (originalData.parts || []).length,
    );
    ...
    for (let i = 0; i < (originalData.parts || []).length; i++) {
        ...
        if (priority === "high" || priority === "medium") {
            console.log(
                `     Part needs ${priority} priority improvement, starting AI call...`,
            );
            const improvedPart = await improvePart(
                part,
                partValidation,
                partEvaluation,
                updateLevel,
            ...
            console.log(
                `     Part ${i + 1} improved in ${Date.now() - partStartTime}ms, changes: ${improvedPart.changes.length}`,
            );
    ...
    for (let i = 0; i < (originalData.chapters || []).length; i++) {
        ...
        if (priority === "high" || priority === "medium") {
            console.log(
                `     Chapter needs ${priority} priority improvement, starting AI call...`,
            );
            const improvedChapter = await improveChapter(
                chapter,
                chapterValidation,
                chapterEvaluation,
                updateLevel,
            ...
            console.log(
                `     Chapter ${i + 1} improved in ${Date.now() - chapterStartTime}ms, changes: ${improvedChapter.changes.length}`,
            );

            if (improvedChapter.changes.length > 0) {
                result.improved.chapters[i] = {
                    ...chapter,
                    ...improvedChapter.data,
                };
    ...
    console.log(
        `   All chapters processed in ${Date.now() - chaptersStartTime}ms`,
    );
    ...
    console.log(
        "   Total scenes to process:",
        (originalData.scenes || []).length,
    );
    ...
    for (let i = 0; i < (originalData.scenes || []).length; i++) {
        ...
        if (priority === "high" || priority === "medium") {
            console.log(
                `     Scene needs ${priority} priority improvement, starting AI call...`,
            );
            const improvedScene = await improveScene(
                scene,
                sceneValidation,
                sceneEvaluation,
                updateLevel,
            ...
            console.log(
                `     Scene ${i + 1} improved in ${Date.now() - sceneStartTime}ms, changes: ${improvedScene.changes.length}`,
            );
    ...
    console.log(
        ...
        (originalData.characters || []).length,
    );
    ...
    for (let i = 0; i < (originalData.characters || []).length; i++) {
        ...
        if (priority === "high" || priority === "medium") {
            console.log(
                `     Character needs ${priority} priority improvement, starting AI call...`,
            );
            const improvedCharacter = await improveCharacter(
                character,
                characterValidation,
                characterEvaluation,
                updateLevel,
            ...
            console.log(
                `     Character ${i + 1} improved in ${Date.now() - characterStartTime}ms, changes: ${improvedCharacter.changes.length}`,
            );

            if (improvedCharacter.changes.length > 0) {
                result.improved.characters[i] = {
                    ...character,
                    ...improvedCharacter.data,
                };
    ...
    console.log(
        `   All characters processed in ${Date.now() - charactersStartTime}ms`,
    );
    ...
    console.log(
        "   Total settings to process:",
        (originalData.settings || []).length,
    );
    ...
    for (let i = 0; i < (originalData.settings || []).length; i++) {
        ...
        if (priority === "high" || priority === "medium") {
            console.log(
                `     Setting needs ${priority} priority improvement, starting AI call...`,
            );
            const improvedSetting = await improveSetting(
                setting,
                settingValidation,
                settingEvaluation,
                updateLevel,
            ...
            console.log(
                `     Setting ${i + 1} improved in ${Date.now() - settingStartTime}ms, changes: ${improvedSetting.changes.length}`,
            );

            if (improvedSetting.changes.length > 0) {
                result.improved.settings[i] = {
                    ...setting,
                    ...improvedSetting.data,
                };
    ...
    console.log(
        `   All settings processed in ${Date.now() - settingsStartTime}ms`,
    );
    ...
    console.log(
        `   Total improvement time: ${Date.now() - improvementStartTime}ms`,
    );
    console.log(`   Total changes made: ${result.summary.totalChanges}`);
    console.log(
        `   Major improvements: ${result.summary.majorImprovements.length}`,
    );
    console.log(
        `   Minor adjustments: ${result.summary.minorAdjustments.length}`,
    );
    console.log(
        "🔧 =================== STORY IMPROVEMENT END ===================\n",
    );
...
// Helper functions
function shouldImproveComponent(
...
function getImprovementPriority(
    ...
    console.log(
        `     Priority check - Errors: ${errorCount}, Warnings: ${warningCount}, Score: ${avgScore}`,
    );
...
Return ONLY the fields that need updating. Keep other fields unchanged.`,
        ...
        if (
            object.dramaticQuestion &&
            object.dramaticQuestion !== story.dramaticQuestion
        ) {
        ...
        return {
            ...
            changes: [],
            improvements: [],
            rationale: "Unable to improve at this time",
        };
    }
}
...
Return ONLY the fields that need updating. Ensure all text fields are reasonable in length.`,
        ...
        if (object.title && object.title.length > 200) {
            ...
            console.log(
                "Warning: Part title truncated from",
                object.title.length,
                "to 200 characters",
            );
...
Return ONLY the fields that need updating.`,
        ...
        if (object.title && object.title.length > 200) {
            sanitizedData.title = object.title.substring(0, 200);
            console.log(
                "Warning: Chapter title truncated from",
                object.title.length,
                "to 200 characters",
            );
        } else if (object.title) {
        ...
        Object.keys(object).forEach((key) => {
            if (key !== "title") {
                ...
                if (typeof value === "string" && value.length > 10000) {
                    ...
                    console.log(
                        `Warning: Chapter ${key} truncated from ${value.length} to 10000 characters`,
                    );
                } else {
...
async function improveScene(
    ...
    try {
        ...
        const structureImprovement = await improveSceneStructure(
            scene,
            issues,
            evaluation,
            updateLevel,
        ...
        const improvedData = { ...structureImprovement.data };
...
function generateImprovementSummary(changes: any, analysisResult: any): any {
    const totalChanges =
        ...
        changes.parts.reduce(
            (sum: number, p: any) => sum + p.fieldsUpdated.length,
            0,
        ) +
        changes.chapters.reduce(
            (sum: number, c: any) => sum + c.fieldsUpdated.length,
            0,
        ) +
        changes.scenes.reduce(
            (sum: number, s: any) => sum + s.fieldsUpdated.length,
            0,
        ) +
        changes.characters.reduce(
            (sum: number, c: any) => sum + c.fieldsUpdated.length,
            0,
        ) +
        changes.settings.reduce(
            (sum: number, s: any) => sum + s.fieldsUpdated.length,
            0,
        );
    ...
    if (
        changes.story.fieldsUpdated.includes("premise") ||
        changes.story.fieldsUpdated.includes("dramaticQuestion")
    ) {
        majorImprovements.push("Strengthened core story elements");
    }

    if (changes.characters.length > 0) {
        const characterCount = changes.characters.filter((c: any) =>
            c.fieldsUpdated.includes("hnsData"),
        ).length;
        if (characterCount > 0) {
            majorImprovements.push(
                `Enhanced ${characterCount} character(s) with development arcs`,
            );
        }
    }

    if (changes.scenes.length > 0) {
        const sceneCount = changes.scenes.filter(
            (s: any) =>
                s.fieldsUpdated.includes("goal") ||
                s.fieldsUpdated.includes("conflict"),
        ).length;
        if (sceneCount > 0) {
            minorAdjustments.push(
                `Clarified goals and conflicts in ${sceneCount} scene(s)`,
            );
        }
    }
    ...
    const unchangedCharacters =
        (analysisResult.originalData?.characters?.length || 0) -
        changes.characters.length;
