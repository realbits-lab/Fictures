 ...
 */
export type StoryImageType = 'story' | 'character' | 'setting' | 'scene' | 'comic-panel';
 ...
 */
export function getAspectRatioForImageType(imageType: StoryImageType): AspectRatio {
...
export async function generateStoryImage(
  ...
  console.log(`[IMAGE-GEN] Starting ${imageType} image generation for story ${storyId}`);
  console.log(`[IMAGE-GEN] Prompt preview: ${prompt.substring(0, 100)}...`);

  // Determine aspect ratio
  const aspectRatio = aspectRatioOverride || getAspectRatioForImageType(imageType);
  ...
  try {
    ...
    console.log(`[IMAGE-GEN] Image size: ${(imageSize / 1024 / 1024).toFixed(2)} MB`);
    ...
    console.log(`[IMAGE-GEN] Created ${optimizedSet.variants.length} optimized variants`);
    ...
    console.error(`[IMAGE-GEN] Error generating ${imageType} image:`, error);
    throw error;
  }
}
...
export function buildStoryImagePrompt(params: {
  ...
  const aspectRatioDesc = aspectRatio === '1:1' ? 'square composition' :
                          aspectRatio === '16:9' ? 'cinematic widescreen composition' :
                          aspectRatio === '9:16' ? 'vertical portrait composition' :
                          aspectRatio === '2:3' ? 'portrait composition' :
