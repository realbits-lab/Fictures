...
export async function generateToonplayWithEvaluation(
  ...
  const improvementHistory: { iteration: number; score: number; passed: boolean }[] = [];
  ...
  console.log(`   Status: ${currentEvaluation.passes ? '✅ PASSES' : '❌ NEEDS IMPROVEMENT'}`);
  ...
  for (let iteration = 1; iteration <= maxIterations; iteration++) {
    ...
    console.log(`   ✅ Improved toonplay generated: ${improvedToonplay.total_panels} panels`);
    ...
    console.log(`   Improvement: ${improvedEvaluation.weighted_score > currentEvaluation!.weighted_score ? '📈 Better' : '📉 Worse'}`);
    console.log(`   Status: ${improvedEvaluation.passes ? '✅ PASSES' : '❌ STILL NEEDS WORK'}`);
  ...
  console.log(`   ═══════════════════════════════════════════════════════════`);
  console.log(`   Best Score: ${bestScore.toFixed(2)}/5.0 (iteration ${improvementHistory.findIndex(h => h.score === bestScore)})`);
  console.log(`   Status: ${bestEvaluation!.passes ? '✅ PASSES' : '⚠️  DOES NOT PASS (using best available)'}`);
...
async function improveToonplay(
  ...
  const { currentToonplay, evaluation, scene, characters, setting, storyGenre, targetPanelCount } = options;
