...
export function formatDialogueEnhanced(text: string): string {
  ...
  for (const line of lines) {
    ...
    if (hasDialogue) {
      ...
      // Pure narrative line
      if (lastWasDialogue && formattedLines.length > 0 && formattedLines[formattedLines.length - 1] !== '') {
        formattedLines.push('');
      }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (hasDialogue) {
      ...
      if (i > 0) {
        ...
        if (prevLine.trim() !== '') {
          issues.push(`Line ${i + 1}: Dialogue not isolated with blank line before`);
          needsFixing = true;
        }
      }
      ...
      if (i < lines.length - 1) {
        ...
        if (nextLine.trim() !== '') {
          issues.push(`Line ${i + 1}: Dialogue not isolated with blank line after`);
          needsFixing = true;
        }
      }

      // Check for multiple dialogue sentences in one line
      const dialogueMatches = line.match(/[""][^"""]*[""](?:\s*[""][^"""]*[""])+/);
      if (dialogueMatches) {
        issues.push(`Line ${i + 1}: Multiple dialogue sentences in single paragraph`);
        needsFixing = true;
      }
      ...
      if (segments.length > 1) {
        issues.push(`Line ${i + 1}: Dialogue mixed with narrative text`);
        needsFixing = true;
      }
    }
  }
