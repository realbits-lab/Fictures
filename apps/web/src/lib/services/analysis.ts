...
export async function getStoryAnalysis(
	...
	const [ratingStats] = await db
		...
		.from(stories)
		.where(and(eq(stories.authorId, userId), inArray(stories.id, storyIds)));
...
async function getStoryPerformanceData(
	...
	return Promise.all(
		storyData.map(async (story) => {
			...
			const reactions =
				Number(likeCounts.find((l) => l.storyId === story.id)?.likes) || 0;
...
export async function getReaderAnalysis(
	...
	return {
		...
		demographics: {
			...
			returningReaders:
				(demographics?.totalReaders || 0) - (demographics?.newReaders || 0),
