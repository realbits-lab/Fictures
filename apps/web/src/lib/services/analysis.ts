...
export async function getStoryAnalysis(
    ...
    if (timeRange === "7d")
        previousPeriod.start.setDate(previousPeriod.start.getDate() - 7);
    ...
    const readerGrowth =
        previousReaderStats?.uniqueReaders > 0
            ? ((readerStats.uniqueReaders - previousReaderStats.uniqueReaders) /
                  previousReaderStats.uniqueReaders) *
              100
            : 0;
    ...
    const [ratingStats] = await db
        ...
        .from(stories)
        .where(
            and(eq(stories.authorId, userId), inArray(stories.id, storyIds)),
        );
    ...
    const engagement =
        readerStats.totalViews > 0
            ? ((totalComments + (Number(likeCounts[0]?.likes) || 0)) /
                  readerStats.totalViews) *
              100
            : 0;
    ...
    return {
        ...
        avgRating: ratingStats.avgRating
            ? Number(ratingStats.avgRating) / 10
            : 0,
        totalComments,
        engagement,
        storiesData,
...
async function getStoryPerformanceData(
    ...
    return Promise.all(
        storyData.map(async (story) => {
            const views =
                viewCounts.find((v) => v.storyId === story.id)?.views || 0;
            const commentsCount =
                commentCounts.find((c) => c.storyId === story.id)?.count || 0;
            const reactions =
                Number(likeCounts.find((l) => l.storyId === story.id)?.likes) ||
                0;
            ...
            const trendPercentage =
                previousViews?.views > 0
                    ? ((views - previousViews.views) / previousViews.views) *
                      100
                    : 0;
...
async function getTrendData(
    ...
    const dailyData = await db
        .select({
            ...
            engagements: count(
                sql`CASE WHEN ${analysisEvents.eventType} IN ('comment_created', 'story_liked', 'share') THEN ${analysisEvents.id} END`,
            ),
    ...
    return dailyData.map((day) => ({
        ...
        views: day.views || 0,
        engagement:
            day.views > 0 ? ((day.engagements || 0) / day.views) * 100 : 0,
...
export async function getReaderAnalysis(
    ...
    return {
        ...
        demographics: {
            ...
            returningReaders:
                (demographics?.totalReaders || 0) -
                (demographics?.newReaders || 0),
...
async function getReadingPatterns(
    ...
    return {
        ...
        avgSessionDuration: sessionStats?.avgDuration
            ? Number(sessionStats.avgDuration)
            : 0,
