    ...
    try {
        ...
        const characterEvaluations = await Promise.all(
            (storyData.characters || []).map((character) =>
                evaluateCharacter(character, storySummary),
            ),
        );

        const settingEvaluations = await Promise.all(
            (storyData.settings || []).map((setting) =>
...
async function evaluateOverallStory(
    storySummary: string,
