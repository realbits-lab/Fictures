...
import { db } from '../db';
import { stories, parts, chapters, scenes, characters, settings } from '../db/schema';
...
const CACHE_KEYS = {
  fullStructure: (storyId: string, userId?: string) =>
    userId ? `story:${storyId}:structure:user:${userId}` : `story:${storyId}:structure:public`,
...
export async function getCachedStoryStructure(
  ...
  if (cached) {
    console.log(`[StoryCache] ✅ HIT: Full structure for ${storyId} (age: ${Date.now() - new Date(cached.cachedAt).getTime()}ms)`);
    return cached;
  }

  console.log(`[StoryCache] ❌ MISS: Full structure for ${storyId} - fetching from DB`);
  ...
  await Promise.all([
    ...
    cache.set(CACHE_KEYS.characterIds(storyId), structure.characterIds, ttl),
    cache.set(CACHE_KEYS.settingIds(storyId), structure.settingIds, ttl),
  ]);

  console.log(`[StoryCache] 💾 SET: Full structure for ${storyId} (TTL: ${ttl}s, ${structure.parts.length} parts, ${structure.chapters.length} chapters, ${structure.scenes.length} scenes)`);
...
async function buildStoryStructure(
  ...
  const sceneIds = [
    ...story.parts.flatMap((p: any) =>
      p.chapters.flatMap((c: any) => c.scenes?.map((s: any) => s.id) || [])
    ),
    ...story.chapters.flatMap((c: any) => c.scenes?.map((s: any) => s.id) || []),
  ];
  ...
  return {
    ...
    ttl: story.status === 'published' ? CACHE_TTL.PUBLISHED_STORY : CACHE_TTL.DRAFT_STORY,
  };
}
...
export async function getCachedEntityIds(storyId: string): Promise<{
  ...
  const [partIds, chapterIds, sceneIds, characterIds, settingIds] = await Promise.all([
...
export async function invalidateStoryCache(storyId: string): Promise<void> {
  ...
  console.log(`[StoryCache] 🗑️ INVALIDATED: All cache entries for story ${storyId}`);
}
...
export async function invalidateCacheForEntity(
  ...
  console.log(`[StoryCache] 🗑️ INVALIDATED: Cache for ${entityType} ${entityId} in story ${storyId}`);
}
...
export async function warmStoryCache(storyIds: string[]): Promise<void> {
  console.log(`[StoryCache] 🔥 Warming cache for ${storyIds.length} stories...`);
  ...
  console.log(`[StoryCache] ✅ Cache warmed for ${storyIds.length} stories in ${duration}ms`);
}
