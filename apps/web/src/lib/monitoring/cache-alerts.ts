...
class CacheMonitor {
    ...
    private checkInvalidationRate(invalidationRate: number): void {
        if (
            invalidationRate > MONITORING_THRESHOLDS.INVALIDATION_RATE_CRITICAL
        ) {
            this.createAlert({
            ...
            invalidationRate > MONITORING_THRESHOLDS.INVALIDATION_RATE_WARNING
        ) {
            this.createAlert({
                type: "high_invalidation_rate",
                severity: "warning",
 ...
 */
export async function startCacheMonitoring(
    ...
    if (!initialHealth.healthy) {
        console.warn(
            `[Cache Monitor] Active alerts: ${initialHealth.alerts.length}`,
        );
    ...
    setInterval(async () => {
        ...
        if (!health.healthy) {
            console.warn(
                `[Cache Monitor] Health check failed - Active alerts: ${health.alerts.length}`,
            );
            ...
            if (criticalAlerts.length > 0) {
                console.error(
                    `[Cache Monitor] 🚨 CRITICAL ALERTS: ${criticalAlerts.length}`,
                );
    ...
    console.log(
        `[Cache Monitor] Monitoring started (checking every ${intervalMs / 1000}s)`,
    );
}
