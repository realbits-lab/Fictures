...
export async function getChapterById(chapterId: string, userId?: string) {
	...
	if (
		chapter.stories?.status !== "published" &&
		chapter.stories?.authorId !== userId
	) {
		return null;
	}
...
export async function getSceneById(sceneId: string, userId?: string) {
	...
	// Check access permissions
	if (
		scene.stories?.status !== "published" &&
		scene.stories?.authorId !== userId
	) {
		return null;
	}
...
export function calculateSceneStatus(scene: {
	content?: string;
	status?: string;
}) {
...
export function calculateChapterStatus(scenes: Array<{ status: string }>) {
	...
	const inProgressScenes = scenes.filter(
		(scene) => scene.status === "in_progress",
	).length;
...
export function calculateStoryStatus(
	...
	const allChapters = [
		...parts.flatMap((part) => [{ status: part.status }]),
		...chapters,
	];
	...
	const completedUnits = allChapters.filter(
		(unit) => unit.status === "completed",
	).length;
	const inProgressUnits = allChapters.filter(
...
export async function getStoryWithStructure(
	...
	const result = await RelationshipManager.getStoryWithStructure(
		storyId,
		includeScenes,
	);
	...
	const dynamicStoryStatus = calculateStoryStatus(
		structuredParts,
		standaloneChapters,
	);
	const finalStoryStatus =
		result.status === "published" ? "published" : dynamicStoryStatus;
...
export async function getChapterWithPart(chapterId: string, userId?: string) {
	...
	// Check access permissions
	if (
		result.story?.status !== "published" &&
		result.story?.authorId !== userId
	) {
		return null;
	}
...
export async function getPublishedStories() {
	...
	const authorIds = [
		...new Set(publishedStories.map((story) => story.authorId)),
	];
...
export async function getCommunityStories() {
	console.log(
		"[getCommunityStories] 🔄 START - Optimized version with aggregation",
	);
	...
	console.log(
		`[getCommunityStories] ✅ Query 1 (stories): ${Date.now() - storiesStart}ms - Found ${publicStories.length} stories`,
	);
	...
	console.log(
		`[getCommunityStories] ✅ Query 2 (stats aggregation): ${Date.now() - statsStart}ms - Found stats for ${communityStats.length} stories`,
	);

	// Create a map for fast lookup
	const statsMap = new Map(communityStats.map((stat) => [stat.storyId, stat]));
	...
	console.log(
		`[getCommunityStories] ✅ Merge completed: ${Date.now() - mergeStart}ms`,
	);
	console.log(
		`[getCommunityStories] ✨ TOTAL TIME: ${Date.now() - startTime}ms (was ~1810ms with N+1 queries)`,
	);
...
export async function getCommunityStory(storyId: string) {
	...
	const storyData = await db
		...
		.limit(1);
	console.log(
		`[getCommunityStory] ✅ Story query: ${Date.now() - storyStart}ms`,
	);
	...
	console.log(
		`[getCommunityStory] ✅ Post count query: ${Date.now() - postsStart}ms`,
	);
	...
	console.log(
		`[getCommunityStory] ✅ Characters query: ${Date.now() - charsStart}ms (${storyCharacters.length} characters)`,
	);
	...
	console.log(
		`[getCommunityStory] ✅ Settings query: ${Date.now() - settingsStart}ms (${storySettings.length} settings)`,
	);

	const totalTime = Date.now() - startTime;
	console.log(
		`[getCommunityStory] ✨ COMPLETE - Total DB time: ${totalTime}ms`,
	);
...
export async function getCommunityPosts(storyId: string) {
	...
	const totalTime = Date.now() - startTime;
	console.log(
		`[getCommunityPosts] ✨ COMPLETE - Total DB time: ${totalTime}ms (${posts.length} posts)`,
	);
