...
export async function getCommunityStoriesForReading() {
    ...
    try {
        ...
        const [publishedStories, authorData, postsCounts] = await Promise.all([
            ...
            db
                .select({
                    ...
                    lastActivity:
                        sql<Date>`MAX(${communityPosts.createdAt})`.as(
                            "lastActivity",
                        ),
                })
                .from(communityPosts)
        ...
        const authorsMap = new Map(
            authorData.map((author) => [author.id, author]),
        );
        ...
        console.log(
            `[PERF-QUERY] 📊 Results: ${storiesWithStats.length} stories`,
        );
