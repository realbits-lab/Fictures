...
export async function storyConceptDevelopment(
    userPrompt: string,
...
genre: [MUST be ONE of: Fantasy, Romance, SciFi, Mystery, Horror, Action, Isekai, LitRPG, Cultivation, Slice, Paranormal, Dystopian, Historical, LGBTQ]
...
export async function generatePartSpecifications(
    storyConcept: Story,
...
Create detailed part specification in YAML format for Part ${storyPart.part}. Include character development, plot progression, themes, and reader engagement elements.`,
        ...
        try {
            ...
            console.error(
                `Failed to parse YAML part specification for Part ${storyPart.part}:`,
                error,
            );
            console.error("Raw text:", text);
            ...
            const fallbackPartSpec: PartSpecification = {
                ...
                words: Math.floor(
                    (storyConcept.words || 60000) / storyConcept.parts.length,
                ),
            ...
            console.log(
                `✅ Created fallback part specification for Part ${storyPart.part}`,
            );
...
export async function generateChapterSpecifications(
    storyConcept: Story,
    partSpec: PartSpecification,
    chapterCount: number = 5,
): Promise<ChapterSpecification[]> {
...
POV character should be the main protagonist: ${Object.keys(storyConcept.chars).find((k) => storyConcept.chars[k].role === "protag")}`,
        ...
        try {
            ...
            console.error(
                `Failed to parse YAML chapter specification for Chapter ${i}:`,
                error,
            );
            console.error("Raw text:", text);
...
export async function generateSceneSpecifications(
    chapterSpec: ChapterSpecification,
    storyCharacters: any[] = [],
    storySettings: any[] = [],
    sceneCount: number = 3,
...
Scene ${i} function within chapter structure. Must include at least one character and one place from the available lists above.`,
        ...
        try {
            ...
            console.error(
                `Failed to parse YAML scene specification for Scene ${i}:`,
                error,
            );
            console.error("Raw text:", text);
...
export async function generateCharacterData(
    ...
    const characterPromises = Object.entries(storyConcept.chars).map(
        async ([key, char]) => {
            try {
                const { text } = await generateText({
                    model: AI_MODELS.writing,
...
Generate comprehensive character details in YAML format.`,
                ...
                try {
                    ...
                    console.error(
                        `Failed to parse character YAML for ${key}:`,
                        yamlError,
                    );
                    ...
                    parsedData = {
                        ...
                        summary:
                            "Character description not available due to parsing error",
                    };
                }
                ...
                console.error(
                    `Failed to generate character data for ${key}:`,
                    error,
                );
...
export async function generatePlaceData(
    storyConcept: Story,
    language: string = "English",
) {
    const allPlaces = [
        ...(storyConcept.setting?.primary || []),
        ...(storyConcept.setting?.secondary || []),
    ];
...
Generate comprehensive location details in YAML format.`,
            ...
            try {
                ...
                console.error(
                    `Failed to parse place YAML for ${place}:`,
                    yamlError,
                );
                // Create fallback parsed data
                parsedData = {
                    ...
                    type: "location",
                    summary:
                        "Place description not available due to parsing error",
                };
            }
...
export async function generateStoryFromPrompt(
    ...
    try {
        ...
        const storyConcept = await storyConceptDevelopment(
            userPrompt,
            language,
        );
        ...
        try {
            ...
            console.log(
                "✅ Optimized variants:",
                imageResult.optimizedSet?.variants.length || 0,
            );
        } catch (error) {
        ...
        for (let partIndex = 0; partIndex < partSpecs.length; partIndex++) {
            ...
            const partWordCount = storyConcept.structure?.dist?.[partIndex]
                ? Math.floor(
                      (storyConcept.words || 60000) *
                          (storyConcept.structure.dist[partIndex] / 100),
                  )
        ...
        const characterData = await generateCharacterData(
            storyConcept,
            language,
        );
