 ...
 */
const getConfig = () => {
  const provider = (process.env.IMAGE_GENERATION_PROVIDER || 'gemini') as ImageProvider;

  return {
    provider,
    gemini: {
      apiKey: process.env.GOOGLE_GENERATIVE_AI_API_KEY || '',
      modelName: process.env.GEMINI_IMAGE_MODEL_NAME || 'imagen-3.0-generate-001',
    },
    aiServer: {
...
class ImageGenerationWrapper {
  ...
  constructor() {
    ...
    if (this.providerType === 'gemini') {
      this.provider = new GeminiImageProvider(config.gemini) as unknown as ImageGenerationProvider;
    } else if (this.providerType === 'ai-server') {
      this.provider = new AIServerImageProvider(config.aiServer) as unknown as ImageGenerationProvider;
    } else {
