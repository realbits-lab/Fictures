...
export const StorySummarySchema = z.object({
	...
	genre: z
		.string()
		.describe(
			"Specific genre classification (e.g., Science Fiction, Mystery, Romance)",
		),
	summary: z
		.string()
		.describe(
	...
	tone: z
		...
		.describe("Overall emotional tone of the story"),
...
export const CharacterSchema = z.object({
	...
	coreTrait: z
		...
		.describe("Defining moral virtue"),
	internalFlaw: z
		.string()
		.describe(
			'Character flaw with cause (format: "[fears/believes/wounded by] X because Y")',
		),
	externalGoal: z.string().describe("What they think will solve their problem"),
	personality: z.object({
		traits: z
			.array(z.string())
			.describe("Behavioral traits (e.g., impulsive, optimistic)"),
		values: z
			.array(z.string())
			.describe("What they care about (e.g., family, honor)"),
	}),
	backstory: z
		.string()
		.describe("Focused history providing motivation context (2-4 paragraphs)"),
	relationships: z
		.record(
			z.string(),
			z.object({
	...
	physicalDescription: z.object({
		...
		distinctiveFeatures: z
			.string()
			.describe('Memorable details for "show don\'t tell"'),
		style: z.string().describe("How they dress/present themselves"),
	}),
	voiceStyle: z.object({
		tone: z
			.string()
			.describe('Speaking tone (e.g., "warm", "sarcastic", "formal")'),
		vocabulary: z
			.string()
			.describe('Word choice level (e.g., "simple", "educated", "technical")'),
		quirks: z.array(z.string()).describe("Verbal tics, repeated phrases"),
		emotionalRange: z
			.string()
			.describe('Emotional expressiveness (e.g., "reserved", "expressive")'),
	}),
	visualStyle: z
		.string()
		.optional()
		.describe("Visual art style for character portrait"),
});
...
export const SettingSchema = z.object({
	...
	adversityElements: z.object({
		...
		scarcityFactors: z
			.array(z.string())
			.describe("Limited resources that force choices"),
	...
	symbolicMeaning: z
		.string()
		.describe("How setting reflects story's moral framework"),
	...
	sensory: z.object({
		...
		taste: z
			.array(z.string())
			.optional()
			.describe("Flavor elements (0-2 items)"),
	}),
	architecturalStyle: z
		.string()
		.optional()
		.describe("Structural design language"),
	visualStyle: z.string().describe("Visual art style for setting image"),
	visualReferences: z
		.array(z.string())
		.describe('Style inspirations (e.g., ["Blade Runner 2049"])'),
	colorPalette: z
		.array(z.string())
		.describe('Dominant colors (e.g., ["warm golds", "dusty browns"])'),
});
...
export const PartSchema = z.object({
	...
	summary: z
		.string()
		.describe("MACRO adversity-triumph arcs with progression planning"),
	...
	characterArcs: z.array(
		z.object({
			...
			macroAdversity: z.object({
				...
				external: z
					.string()
					.describe("External obstacle forcing internal confrontation"),
			}),
			macroVirtue: z
				.string()
				.describe("From Character.coreTrait - THE defining moral choice"),
			macroConsequence: z.string().describe("Earned payoff for virtue"),
			macroNewAdversity: z
				.string()
				.describe("How resolution creates next act's challenge"),
			estimatedChapters: z
				.number()
				.describe("Number of chapters for this arc (2-4 typical)"),
			arcPosition: z
				.enum(["primary", "secondary"])
				.describe("Primary arcs get more chapters"),
...
export const ChapterSchema = z.object({
	...
	arcPosition: z
		...
		.describe("Position in macro arc"),
	contributesToMacroArc: z
		.string()
		.describe("How this advances the macro transformation"),
	focusCharacters: z
		.array(z.string())
		.describe("Character IDs appearing in this chapter"),
	adversityType: z
		.enum(["internal", "external", "both"])
		.describe("Type of adversity"),
	virtueType: z
		.enum([
			"courage",
			"compassion",
	...
	seedsPlanted: z
		...
		.optional()
		.describe("Seeds for future payoff"),
	seedsResolved: z
		.array(
			z.object({
		...
		.optional()
		.describe("Seeds resolved in this chapter"),
	connectsToPreviousChapter: z
		.string()
		.describe("How previous resolution created this adversity"),
	createsNextAdversity: z
		.string()
		.describe("How this resolution creates next problem"),
...
export const SceneSummarySchema = z.object({
	...
	summary: z
		.string()
		.describe("Scene specification: what happens, emotional beat, purpose"),
	cyclePhase: z
		.enum(["setup", "confrontation", "virtue", "consequence", "transition"])
		.describe("Adversity-triumph cycle phase"),
	emotionalBeat: z
		.enum([
			"fear",
			"hope",
	...
	characterFocus: z
		.array(z.string())
		.describe("Character IDs appearing in this scene"),
	settingId: z
		.string()
		.optional()
		.describe("Setting ID where this scene takes place"),
	sensoryAnchors: z
		.array(z.string())
		.describe("Key sensory details to include"),
	dialogueVsDescription: z
		.string()
		.describe('Balance guidance (e.g., "60% dialogue, 40% description")'),
	suggestedLength: z
		.enum(["short", "medium", "long"])
		.describe(
			"Scene length (short: 300-500, medium: 500-800, long: 800-1000 words)",
		),
