		...
		for (const part of parts) {
			for (let i = 0; i < chaptersPerPart; i++) {
				...
				onProgress({
					...
					data: {
						currentItem: chapterIndex,
						totalItems: partsCount * chaptersPerPart,
					},
				});
		...
		for (const chapter of chapters) {
			...
			for (let i = 0; i < scenesPerChapter; i++) {
				onProgress({
					...
					data: {
						currentItem: scenesWithSummaries.length + 1,
						totalItems: chapters.length * scenesPerChapter,
					},
				});
		...
		for (const chapter of chapters) {
			...
			for (const sceneSummary of chapterScenes) {
				...
				const character = characters.find((c) =>
					sceneSummary.characterFocus?.includes(c.id),
				);
