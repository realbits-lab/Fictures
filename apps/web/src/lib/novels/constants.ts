...
export function validateGenerationConstraints(options: {
  ...
  if (options.characterCount !== undefined) {
    if (options.characterCount < NOVEL_GENERATION_CONSTRAINTS.CHARACTER.MIN) {
      errors.push(`Character count must be at least ${NOVEL_GENERATION_CONSTRAINTS.CHARACTER.MIN}`);
    }
    if (options.characterCount > NOVEL_GENERATION_CONSTRAINTS.CHARACTER.MAX) {
      errors.push(`Character count must not exceed ${NOVEL_GENERATION_CONSTRAINTS.CHARACTER.MAX}`);
    }
  }

  if (options.settingCount !== undefined) {
    if (options.settingCount < NOVEL_GENERATION_CONSTRAINTS.SETTING.MIN) {
      errors.push(`Setting count must be at least ${NOVEL_GENERATION_CONSTRAINTS.SETTING.MIN}`);
    }
    if (options.settingCount > NOVEL_GENERATION_CONSTRAINTS.SETTING.MAX) {
      errors.push(`Setting count must not exceed ${NOVEL_GENERATION_CONSTRAINTS.SETTING.MAX}`);
    }
  }

  if (options.partsCount !== undefined) {
    if (options.partsCount < NOVEL_GENERATION_CONSTRAINTS.PARTS.MIN) {
      errors.push(`Parts count must be at least ${NOVEL_GENERATION_CONSTRAINTS.PARTS.MIN}`);
    }
    if (options.partsCount > NOVEL_GENERATION_CONSTRAINTS.PARTS.MAX) {
      errors.push(`Parts count must not exceed ${NOVEL_GENERATION_CONSTRAINTS.PARTS.MAX}`);
    }
  }

  if (options.chaptersPerPart !== undefined) {
    if (options.chaptersPerPart < NOVEL_GENERATION_CONSTRAINTS.CHAPTERS_PER_PART.MIN) {
      errors.push(`Chapters per part must be at least ${NOVEL_GENERATION_CONSTRAINTS.CHAPTERS_PER_PART.MIN}`);
    }
    if (options.chaptersPerPart > NOVEL_GENERATION_CONSTRAINTS.CHAPTERS_PER_PART.MAX) {
      errors.push(`Chapters per part must not exceed ${NOVEL_GENERATION_CONSTRAINTS.CHAPTERS_PER_PART.MAX}`);
    }
  }

  if (options.scenesPerChapter !== undefined) {
    if (options.scenesPerChapter < NOVEL_GENERATION_CONSTRAINTS.SCENES_PER_CHAPTER.MIN) {
      errors.push(`Scenes per chapter must be at least ${NOVEL_GENERATION_CONSTRAINTS.SCENES_PER_CHAPTER.MIN}`);
    }
    if (options.scenesPerChapter > NOVEL_GENERATION_CONSTRAINTS.SCENES_PER_CHAPTER.MAX) {
      errors.push(`Scenes per chapter must not exceed ${NOVEL_GENERATION_CONSTRAINTS.SCENES_PER_CHAPTER.MAX}`);
    }
  }
...
export function applyGenerationDefaults(options: {
  ...
  return {
    characterCount: options.characterCount ?? NOVEL_GENERATION_CONSTRAINTS.CHARACTER.DEFAULT,
    settingCount: options.settingCount ?? NOVEL_GENERATION_CONSTRAINTS.SETTING.DEFAULT,
    partsCount: options.partsCount ?? NOVEL_GENERATION_CONSTRAINTS.PARTS.DEFAULT,
    chaptersPerPart: options.chaptersPerPart ?? NOVEL_GENERATION_CONSTRAINTS.CHAPTERS_PER_PART.DEFAULT,
    scenesPerChapter: options.scenesPerChapter ?? NOVEL_GENERATION_CONSTRAINTS.SCENES_PER_CHAPTER.DEFAULT,
  };
}
