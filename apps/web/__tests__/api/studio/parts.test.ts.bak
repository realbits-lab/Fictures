...
describe("Parts API", () => {
	...
	beforeAll(async () => {
		...
		console.log(`✅ Characters generated: ${charactersData.characters.length}`);
	...
	it("should generate parts via POST /studio/api/parts", async () => {
		...
		const successData: GeneratePartsResponse = data as GeneratePartsResponse;
		...
		// ============================================================================
		const { parts }: { parts: GeneratePartsResponse["parts"] } = successData;
		...
		for (const part of parts) {
			...
			expect(
				part.characterArcs === null || typeof part.characterArcs === "object",
			).toBe(true);
