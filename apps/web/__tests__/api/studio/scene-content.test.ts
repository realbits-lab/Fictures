...
describe("Scene Content API", () => {
    ...
    it("should update scene content via POST /studio/api/scene-content", async () => {
        const requestBody = {
            sceneId: testSceneId,
            content:
                "The hero stood at the edge of the cliff, gazing into the abyss below. The wind howled around him, carrying whispers of ancient prophecies.",
        };
        ...
        console.log(
            `  Content length: ${data.scene.content.length} characters`,
        );
    }, 10000);
});
