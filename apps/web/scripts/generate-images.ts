...
import { stories, characters, settings, scenes, chapters } from '../drizzle/schema';
...
const typeArg = getArgValue('--type') as 'story' | 'character' | 'setting' | 'scene' | undefined;
...
async function main() {
  try {
    ...
    if (storyIdArg) {
      ...
      story = await db.query.stories.findFirst();
      if (!story) {
        throw new Error('No stories found in database. Create a story first.');
      }
    }
    ...
    if (!typeArg || typeArg === 'story') {
      ...
      try {
        ...
        if (validation.isValid) {
          ...
          console.log(`      Dimensions: ${validation.dimensions.width}×${validation.dimensions.height}`);
          console.log(`      Aspect Ratio: ${validation.actualRatio}`);
          validImages++;
        } else {
          console.log(`   ❌ Validation FAILED`);
          console.log(`      Dimensions: ${validation.dimensions.width}×${validation.dimensions.height} (expected ${validation.expectedSpec.width}×${validation.expectedSpec.height})`);
          console.log(`      Aspect Ratio: ${validation.actualRatio} (expected ${validation.expectedSpec.aspectRatio})`);
      ...
      for (const char of storyCharacters) {
        ...
        try {
          const imageUrl = await generateImage({
            ...
            targetData: {
              name: char.name,
              physicalDescription: char.physicalDescription || { appearance: char.summary || '', distinctiveFeatures: '', style: '' },
        ...
        try {
          const imageUrl = await generateImage({
            ...
            targetData: {
              ...
              mood: setting.mood,
              sensory: setting.sensory || { sight: [], sound: [] },
    ...
    if (!typeArg || typeArg === 'scene') {
      ...
      for (const chapter of storyChapters) {
        ...
        console.log(`   Chapter "${chapter.title}": ${chapterScenes.length} scene(s)`);
        console.log();
        ...
        for (const scene of chapterScenes) {
          ...
          try {
            ...
            const filename = `scene_${scene.id}_${scene.title.replace(/[^a-z0-9]/gi, '_').toLowerCase().substring(0, 30)}.png`;
            const filepath = path.join(OUTPUT_DIR, filename);
            const buffer = await downloadImage(imageUrl, filepath);
            console.log(`   ✅ Saved: ${filename}`);
