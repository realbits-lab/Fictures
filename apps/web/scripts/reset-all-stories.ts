...
if (!confirmFlag) {
  ...
  console.log('  dotenv --file .env.local run node scripts/reset-all-stories.mjs --confirm');
  console.log();
  process.exit(0);
}
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 401) {
        ...
        console.error();
        console.error('💡 Run: dotenv --file .env.local run node scripts/setup-auth-users.mjs');
        process.exit(1);
      }
      ...
      if (response.status === 403) {
        ...
        console.error();
        console.error('💡 This operation requires a manager account with admin:all scope');
        process.exit(1);
      }
    ...
    console.log(`  • Stories:         ${report.database.stories.toLocaleString()}`);
    ...
    console.log(`  • AI Interactions: ${report.database.aiInteractions.toLocaleString()}`);
    console.log();
    ...
    const logFile = path.join(logDir, `reset-all-${new Date().toISOString().replace(/:/g, '-').split('.')[0]}.json`);
