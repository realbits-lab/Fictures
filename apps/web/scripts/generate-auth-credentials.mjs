    ...
    for (const env of environments) {
        ...
        for (const profile of profiles) {
            if (!authData[env].profiles[profile]) {
                console.warn(
                    `⚠️  Profile "${profile}" not found in environment "${env}"`,
                );
                continue;
            }
    ...
    for (const [env, data] of Object.entries(updates)) {
        ...
        for (const [profile, creds] of Object.entries(data.profiles)) {
            ...
            const keyWithoutPrefix = creds.apiKey.startsWith(expectedPrefix)
                ...
                : creds.apiKey;
            const isValid =
                creds.apiKey.startsWith(expectedPrefix) &&
                keyWithoutPrefix.length >= 40;
            console.log(
...
async function main() {
    ...
    console.log(
        `  Mode:         ${options.outputOnly ? "Output Only" : options.dryRun ? "Dry Run" : "Live Update"}`,
    );
    console.log("");
    ...
    try {
        ...
        if (options.dryRun) {
            ...
            console.log(
                `✅ Successfully updated credentials in ${AUTH_FILE_PATH}\n`,
            );
        }
    } catch (error) {
