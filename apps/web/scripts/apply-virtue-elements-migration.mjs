...
if (!DATABASE_URL) {
  console.error("❌ DATABASE_URL or DATABASE_URL_UNPOOLED environment variable not set");
  process.exit(1);
}
...
async function applyMigration() {
  ...
  try {
    ...
    if (result.length > 0) {
      ...
      console.log("⚠️  Warning: virtue_elements column not found after migration");
    }
  } catch (error) {
