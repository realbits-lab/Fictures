...
if (!storyId) {
  ...
  console.error();
  console.error('Usage:');
  console.error('  dotenv --file .env.local run pnpm exec tsx scripts/remove-story.ts STORY_ID [--confirm]');
  console.error();
  console.error('Examples:');
  console.error('  # Preview mode');
  console.error('  dotenv --file .env.local run pnpm exec tsx scripts/remove-story.ts story_abc123');
  console.error();
  console.error('  # Execute removal');
  console.error('  dotenv --file .env.local run pnpm exec tsx scripts/remove-story.ts story_abc123 --confirm');
  console.error();
  process.exit(1);
}
...
if (!confirmFlag) {
  ...
  console.log();
  console.log(`This script will permanently delete story "${storyId}" and all related data:`);
  ...
  console.log(`  dotenv --file .env.local run pnpm exec tsx scripts/remove-story.ts ${storyId} --confirm`);
  console.log();
  process.exit(0);
}
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 401) {
        ...
        console.error();
        console.error('💡 Run: dotenv --file .env.local run pnpm exec tsx scripts/setup-auth-users.ts');
        process.exit(1);
      }

      if (response.status === 403) {
        console.error('❌ Insufficient permissions');
        console.error(`   ${result.message || 'stories:write or admin:all scope required'}`);
        console.error();
        console.error('💡 This operation requires a writer or manager account');
        process.exit(1);
      }
    ...
    console.log(`  • Parts:           ${report.database.parts.toLocaleString()}`);
    console.log(`  • Chapters:        ${report.database.chapters.toLocaleString()}`);
    console.log(`  • Scenes:          ${report.database.scenes.toLocaleString()}`);
    console.log(`  • Characters:      ${report.database.characters.toLocaleString()}`);
    console.log(`  • Settings:        ${report.database.settings.toLocaleString()}`);
    console.log();
