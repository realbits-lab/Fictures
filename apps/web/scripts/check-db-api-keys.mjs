    ...
    `;
        ...
        for (const [email, keys] of Object.entries(keysByEmail)) {
            ...
            keys.forEach((key, idx) => {
                console.log(
                    `   ${idx === 0 ? "Latest" : "Old   "}: ${key.key_prefix}... (prefix only, key is hashed)`,
                );
                ...
                console.log(
                    `   Created: ${new Date(key.created_at).toISOString()}`,
                );
                console.log(
                    `   Last used: ${key.last_used_at ? new Date(key.last_used_at).toISOString() : "never"}`,
                );
                console.log(
                    `   Expires: ${key.expires_at ? new Date(key.expires_at).toISOString() : "never"}`,
                );
                console.log("");
            });
            ...
            if (authFileKey) {
                ...
                console.log(
                    `   ${prefixMatches ? "✅ PREFIX MATCH" : "❌ PREFIX MISMATCH"}`,
                );

                if (!prefixMatches) {
                    console.log(
                        `   ⚠️  WARNING: Auth file key prefix does not match database!`,
                    );
                    console.log(
                        `   This means the auth file key is NOT the same as the database key.`,
                    );
                }
                console.log("");
            } else {
        ...
        console.log("\n📋 Summary:");
        console.log(
            `   Total users with API keys: ${Object.keys(keysByEmail).length}`,
        );
        console.log(`   Total API keys: ${result.length}`);
        console.log(
            `   Active keys: ${result.filter((k) => k.is_active).length}`,
        );
        console.log("");
        console.log(
            "ℹ️  Note: API keys in database are stored as hashes for security.",
        );
        console.log(
