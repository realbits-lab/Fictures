...
const IMAGE_TESTS: ImageTestConfig[] = [
	{
		name: "story_cover",
		prompt:
			"A mysterious book cover with magical elements, cinematic widescreen composition, dramatic lighting",
		aspectRatio: "16:9",
	},
	{
		name: "character_portrait",
		prompt:
			"Portrait of a young warrior with determined expression, square composition, detailed facial features",
		aspectRatio: "1:1",
	},
	{
		name: "setting_visual",
		prompt:
			"Ancient library with towering bookshelves, mysterious atmosphere, square composition",
		aspectRatio: "1:1",
	},
	{
		name: "scene_illustration",
		prompt:
			"Epic battle scene in a fantasy castle, cinematic widescreen, dramatic action",
		aspectRatio: "16:9",
	},
];
	...
	try {
		...
		for (const config of IMAGE_TESTS) {
			...
			try {
				...
				const buffer = await downloadAndSaveImage(result.imageUrl, filename);
				console.log(`      ✅ Saved: ${filename}`);

				// Validate
				const validation = await validateImage(buffer, config.aspectRatio);
