 ...
 */
async function downloadAndSaveImage(imageUrl: string, filename: string): Promise<Buffer> {
 ...
 * Validate image
 */
async function validateImage(buffer: Buffer, expectedRatio: string): Promise<{
  width: number;
  height: number;
  actualRatio: string;
  isValid: boolean;
...
async function main() {
  try {
    let totalTests = IMAGE_TESTS.length;
    let passed = 0;
    let failed = 0;
