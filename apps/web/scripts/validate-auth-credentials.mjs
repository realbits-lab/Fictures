...
function validatePassword(password) {
  ...
  if (password.length < rules.minLength) {
    errors.push(`Password too short (${password.length} chars, expected ${rules.minLength})`);
  } else if (password.length > rules.maxLength) {
    errors.push(`Password too long (${password.length} chars, expected ${rules.maxLength})`);
  }
  ...
  if (rules.requireSpecial) {
    ...
    if (!hasSpecial) {
      errors.push(`Missing special characters (allowed: ${rules.allowedSpecialChars})`);
    }
  }
  ...
  const score = {
    ...
    hasSpecial: new RegExp(`[${rules.allowedSpecialChars.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}]`).test(password),
...
function validateApiKey(apiKey) {
  ...
  const invalidChars = keyPart.split('').filter(char => !rules.allowedChars.includes(char));
  ...
  const entropy = Math.log2(Math.pow(rules.allowedChars.length, keyPart.length));
  const quality = uniqueChars >= keyPart * 0.7 ? 'Good' : uniqueChars >= keyPart * 0.5 ? 'Fair' : 'Poor';
...
function validateEmail(email) {
  ...
  return {
    ...
    errors: [
      ...
      ...(!hasCorrectDomain ? [`Expected domain @${expectedDomain}`] : []),
    ],
  };
}
...
function validateCredentials(options = {}) {
  ...
  for (const env of environments) {
    ...
    for (const profile of profileNames) {
      ...
      const hasWarnings = !emailValidation.hasCorrectDomain ||
      ...
      results.environments[env].profiles[profile] = {
        ...
        overall: {
          valid: isValid,
          hasWarnings,
          status: isValid ? (hasWarnings ? 'Valid (with warnings)' : 'Valid') : 'Invalid',
        },
      };
    }
  }
...
function displayResults(results, verbose = false) {
    ...
    for (const [profile, data] of Object.entries(envData.profiles)) {
      ...
      if (verbose || !data.overall.valid || data.overall.hasWarnings) {
        ...
        if (data.email.errors.length > 0) {
          data.email.errors.forEach(err => console.log(`       ❌ ${err}`));
        }
        ...
        if (data.password.errors.length > 0) {
          data.password.errors.forEach(err => console.log(`       ❌ ${err}`));
        }

        // API Key
        console.log(`     API Key: ${data.apiKey.value}`);
        console.log(`       Length: ${data.apiKey.length} chars (after prefix)`);
        console.log(`       Quality: ${data.apiKey.quality}`);
        if (verbose) {
          console.log(`       Entropy: ${data.apiKey.entropy} bits`);
          console.log(`       Unique chars: ${data.apiKey.uniqueChars}/${data.apiKey.length}`);
        }
        if (data.apiKey.errors.length > 0) {
          data.apiKey.errors.forEach(err => console.log(`       ❌ ${err}`));
        }
      }
    }
  ...
  if (results.summary.invalid === 0) {
    ...
    if (results.summary.warnings > 0) {
      console.log(`⚠️  However, ${results.summary.warnings} profile(s) have warnings. Run with --verbose for details.`);
    }
  } else {
    console.log(`❌ Found ${results.summary.invalid} invalid credential(s). Please fix them.`);
  }
  console.log('');
}
...
async function main() {
  ...
  try {
    ...
    if (results.summary.invalid > 0 && options.fix) {
      console.log('🔧 Auto-fix requested. Regenerating invalid credentials...\n');
      generateCredentials();
      console.log('✅ Credentials regenerated. Please run validation again to verify.\n');
    }
