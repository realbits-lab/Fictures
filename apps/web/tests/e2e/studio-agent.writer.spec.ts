...
test.describe("Studio Agent - Writer", () => {
    ...
    test.describe("Navigation Tests", () => {
        test("TC-AGENT-NAV-001: Agent chat interface accessible from Studio", async ({
            page,
        }) => {
            const agentButton = page.locator(
                '[data-testid="agent-button"], button:has-text("Agent")',
            );
        ...
        test("TC-AGENT-NAV-003: Agent panel toggles open/close", async ({
            page,
        }) => {
    ...
    test.describe("Content Tests", () => {
        test("TC-AGENT-CONTENT-001: Chat interface displays correctly", async ({
            page,
        }) => {
            const agentButton = page.locator('[data-testid="agent-button"]');
        ...
        test("TC-AGENT-CONTENT-004: Empty state shows welcome message", async ({
            page,
        }) => {
            const agentButton = page.locator('[data-testid="agent-button"]');
    ...
    test.describe("Functionality Tests", () => {
        test("TC-AGENT-FUNC-001: Send message to agent works", async ({
            page,
        }) => {
            const agentButton = page.locator('[data-testid="agent-button"]');
            ...
            if (await agentButton.isVisible()) {
                ...
                if (await messageInput.isVisible()) {
                    await messageInput.fill(
                        "Hello, can you help me write a scene?",
                    );
    ...
    test.describe("Performance Tests", () => {
        test("TC-AGENT-PERF-001: Agent panel opens in under 500ms", async ({
            page,
        }) => {
            const agentButton = page.locator('[data-testid="agent-button"]');
            ...
            if (await agentButton.isVisible()) {
                ...
                await agentButton.click();
                await page
                    .locator('[data-testid="agent-panel"]')
