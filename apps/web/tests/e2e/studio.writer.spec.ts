...
test.describe('Studio Page - Writer', () => {
  ...
  test.describe('Navigation Tests', () => {
    test('TC-STUDIO-NAV-001: Studio menu item highlighted when active', async ({ page }) => {
      const studioNav = page.locator('nav a[href="/studio"]');
      await expect(studioNav).toHaveClass(/active|current/);
    });

    test('TC-STUDIO-NAV-002: Clicking Studio navigates correctly', async ({ page }) => {
    ...
    test('TC-STUDIO-NAV-003: Back navigation returns to previous page', async ({ page }) => {
      await page.goto('/novels');
      await page.goto('/studio');
      await page.goBack();
      await page.waitForLoadState('networkidle');
    ...
    test('TC-STUDIO-NAV-004: Story card click navigates to editor', async ({ page }) => {
  ...
  test.describe('Content Tests', () => {
    test('TC-STUDIO-CONTENT-001: Story list displays user\'s stories', async ({ page }) => {
      const storyList = page.locator('[data-testid="story-list"]');
      await expect(storyList).toBeVisible();
    });

    test('TC-STUDIO-CONTENT-002: "Create New Story" button visible', async ({ page }) => {
      const createButton = page.locator('button:has-text("Create"), a:has-text("New Story")');
      await expect(createButton.first()).toBeVisible();
    });

    test('TC-STUDIO-CONTENT-003: Story cards show correct information', async ({ page }) => {
    ...
    test('TC-STUDIO-CONTENT-004: Empty state shows when no stories', async ({ page, request }) => {
  ...
  test.describe('Functionality Tests', () => {
    test('TC-STUDIO-FUNC-001: Create new story button opens creation flow', async ({ page }) => {
      const createButton = page.locator('button:has-text("Create"), a:has-text("New Story")');
    ...
    test('TC-STUDIO-FUNC-002: Story card click navigates to editor', async ({ page }) => {
      const storyCard = page.locator('[data-testid="story-card"]').first();
    ...
    test('TC-STUDIO-FUNC-005: View toggle (card/table) works correctly', async ({ page }) => {
    ...
    test('TC-STUDIO-FUNC-006: Story search/filter functions properly', async ({ page }) => {
  ...
  test.describe('Performance Tests', () => {
    test('TC-STUDIO-PERF-001: Page loads in under 2 seconds', async ({ page }) => {
    ...
    test('TC-STUDIO-PERF-002: Story list renders smoothly with many stories', async ({ page }) => {
      const storyCards = page.locator('[data-testid="story-card"]');
      const count = await storyCards.count();
  ...
  test.describe('Error Handling Tests', () => {
    test('TC-STUDIO-ERROR-003: Network errors don\'t crash the page', async ({ page }) => {
