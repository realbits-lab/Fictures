...
test.describe('Community Page', () => {
  ...
  test.describe('Content Tests', () => {
    test('TC-COMMUNITY-CONTENT-001: Community posts display correctly', async ({ page }) => {
      const postsList = page.locator('[data-testid="posts-list"]');
      await expect(postsList).toBeVisible();
    });

    test('TC-COMMUNITY-CONTENT-002: Post cards show author and timestamp', async ({ page }) => {
  ...
  test.describe('Performance Tests', () => {
    test('TC-COMMUNITY-PERF-001: Page loads in under 2 seconds', async ({ page }) => {
