...
test.describe("Home Page", () => {
	test.describe("Redirect Tests", () => {
		test("TC-HOME-REDIRECT-001: Anonymous users redirected from / to /novels", async ({
			page,
		}) => {
			await page.goto("/");
		...
		test("TC-HOME-REDIRECT-002: Authenticated users redirected from / to /novels", async ({
			page,
			context,
		}) => {
		...
		test("TC-HOME-REDIRECT-003: Redirect uses proper HTTP status code", async ({
			page,
		}) => {
		...
		test("TC-HOME-REDIRECT-004: Redirect preserves query parameters", async ({
			page,
		}) => {
	...
	test.describe("Performance Tests", () => {
		test("TC-HOME-PERF-001: Redirect completes in under 100ms", async ({
			page,
		}) => {
		...
		test("TC-HOME-PERF-002: No JavaScript errors during redirect", async ({
			page,
		}) => {
