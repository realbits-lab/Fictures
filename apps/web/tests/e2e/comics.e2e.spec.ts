...
test.describe('Comics Page', () => {
  ...
  test.describe('Navigation Tests', () => {
    test('TC-COMICS-NAV-001: Comics menu item highlighted when active', async ({ page }) => {
      const comicsNav = page.locator('nav a[href="/comics"]');
      await expect(comicsNav).toHaveClass(/active|current/);
    });

    test('TC-COMICS-NAV-003: Comic card click opens reader', async ({ page }) => {
  ...
  test.describe('Access Control Tests', () => {
    test('TC-COMICS-AUTH-001: Anonymous users can access page', async ({ page }) => {
  ...
  test.describe('Content Tests', () => {
    test('TC-COMICS-CONTENT-001: Published comics display correctly', async ({ page }) => {
  ...
  test.describe('Performance Tests', () => {
    test('TC-COMICS-PERF-001: Comic grid loads in under 2 seconds', async ({ page }) => {
