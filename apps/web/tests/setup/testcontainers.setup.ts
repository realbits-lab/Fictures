...
// Import enum definitions from database schema (SSOT)
...
import { getAllTestData, TEST_IDS } from "../helpers/static-novel-data";
...
export async function runMigrations(dbUrl: string): Promise<void> {
    ...
    // Helper to format array as PostgreSQL enum values
    const formatEnumValues = (values: readonly string[]): string =>
        values.map((v) => `'${v}'`).join(", ");

    try {
        // Create enums using pgEnum definitions from database schema (SSOT)
        const genreValues = formatEnumValues(genre.enumValues);
        await client.unsafe(`
			DO $$ BEGIN
				CREATE TYPE genre AS ENUM (${genreValues});
			EXCEPTION
				WHEN duplicate_object THEN null;
			END $$;
		`);

        const toneValues = formatEnumValues(tone.enumValues);
        await client.unsafe(`
			DO $$ BEGIN
				CREATE TYPE tone AS ENUM (${toneValues});
			EXCEPTION
				WHEN duplicate_object THEN null;
			END $$;
		`);

        const statusValues = formatEnumValues(status.enumValues);
        await client.unsafe(`
			DO $$ BEGIN
				CREATE TYPE status AS ENUM (${statusValues});
			EXCEPTION
				WHEN duplicate_object THEN null;
			END $$;
		`);

        const characterRoleValues = formatEnumValues(characterRole.enumValues);
        await client.unsafe(`
			DO $$ BEGIN
				CREATE TYPE character_role AS ENUM (${characterRoleValues});
			EXCEPTION
				WHEN duplicate_object THEN null;
			END $$;
		`);
        ...
        // Create chapters table (simplified for testing - no status field in actual schema)
        await client`
        ...
        // Create scenes table (uses status enum for novel_status)
        await client`
			CREATE TABLE IF NOT EXISTS scenes (
				...
				image_url TEXT,
				image_variants JSONB,
				novel_status status DEFAULT 'draft' NOT NULL,
...
export async function seedTestData(dbUrl: string): Promise<void> {
    ...
    try {
        ...
        if (writerUsers.length === 0) {
            throw new Error(
                "Writer user not found in database. Ensure setup-auth-users.ts ran successfully.",
            );
        }
        ...
        // 5. Insert chapters (no status field in actual schema)
        for (const chapter of testData.chapters) {
            await client`
				INSERT INTO chapters (
					id, story_id, part_id, title, chapter_number, summary,
					created_at, updated_at
				)
				VALUES (
					${chapter.id},
        ...
        // 6. Insert scenes (uses novel_status column)
        for (const scene of testData.scenes) {
            await client`
				INSERT INTO scenes (
					id, chapter_id, scene_number, title, summary, content,
					image_url, image_variants, novel_status, published_at,
