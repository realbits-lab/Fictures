...
test.describe('Network Error Handling', () => {
  test('TC-ERROR-001: Network errors display user-friendly messages', async ({ page }) => {
    await page.goto('/');
  ...
  test('TC-ERROR-003: API errors don\'t crash the application', async ({ page }) => {
    await page.goto('/studio');
    await page.waitForLoadState('networkidle');
  ...
  test('TC-ERROR-004: Error boundaries catch React errors', async ({ page }) => {
    const consoleErrors: string[] = [];
  ...
  test('TC-ERROR-007: Retry mechanisms work for transient errors', async ({ page }) => {
    let attemptCount = 0;
