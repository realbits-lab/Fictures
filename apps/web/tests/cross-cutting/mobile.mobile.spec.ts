...
test.describe("Mobile Responsiveness", () => {
    ...
    test("TC-MOBILE-001: All pages render correctly on mobile (320px-768px)", async ({
        page,
    }) => {
        const pages = [
            "/",
    ...
    test("TC-MOBILE-003: Touch interactions work correctly", async ({
        page,
    }) => {
    ...
    test("TC-MOBILE-004: Content is readable on small screens", async ({
        page,
    }) => {
        await page.goto("/novels");
        await page.waitForLoadState("networkidle");
