...
const TEST_PROMPT_IDS = values.prompts
  ...
  : ['last-garden'];
const HYPOTHESIS = values.hypothesis || 'Experiment version will improve metrics';

// Output configuration
const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
const OUTPUT_FILE = values.output || `results/ab-test-${CONTROL_VERSION}-vs-${EXPERIMENT_VERSION}-${timestamp}.json`;
 ...
 */
async function loadOrGenerateResults(version: string, prompts: string[], sampleSize: number): Promise<TestStoryResult[]> {
  ...
  try {
    ...
    const filteredResults = suiteResult.stories.filter((story: TestStoryResult) => {
    ...
    console.log(`  → No existing results found for ${version}, will generate new ones`);
  }
...
function calculateTTest(control: number[], experiment: number[]): number {
  ...
  const t = (experimentStats.mean - controlStats.mean) /
    Math.sqrt((controlStats.variance / control.length) + (experimentStats.variance / experiment.length));

  // Degrees of freedom (Welch-Satterthwaite equation)
  const df = Math.pow(
    (controlStats.variance / control.length) + (experimentStats.variance / experiment.length),
    2
  ) / (
    Math.pow(controlStats.variance / control.length, 2) / (control.length - 1) +
    Math.pow(experimentStats.variance / experiment.length, 2) / (experiment.length - 1)
  );
...
function makeRecommendation(
  ...
  if (pValue > 0.05) {
    console.log(`  ⚠ Results not statistically significant (p=${pValue.toFixed(3)})`);
    return 'REVERT';
  }
  ...
  console.log(`  → Improvements: ${improvements}, Regressions: ${regressions}, Major: ${majorRegressions}`);
...
async function main() {
  ...
  const controlResults = await loadOrGenerateResults(CONTROL_VERSION, TEST_PROMPT_IDS, SAMPLE_SIZE);
  ...
  const experimentResults = await loadOrGenerateResults(EXPERIMENT_VERSION, TEST_PROMPT_IDS, SAMPLE_SIZE);
  ...
  const keyMetrics = ['gamdongAchievement', 'sceneQualityScore', 'virtueSceneWordCount'];
  const controlValues = controlResults.map(r => r.corePrincipleScores.emotionalResonance);
  const experimentValues = experimentResults.map(r => r.corePrincipleScores.emotionalResonance);
...
═══════════════════════════════════════════════════════════════
  ...
  TOP IMPROVEMENTS:
${metricDeltas.slice(0, 3).map(d =>
    `  • ${d.metric}: ${(d.baseline * 100).toFixed(0)}% → ${(d.experiment * 100).toFixed(0)}% (${d.percentage > 0 ? '+' : ''}${d.percentage.toFixed(1)}%)`
  ).join('\n')}

  TOP REGRESSIONS:
${metricDeltas.slice(-3).map(d =>
    `  • ${d.metric}: ${(d.baseline * 100).toFixed(0)}% → ${(d.experiment * 100).toFixed(0)}% (${d.percentage > 0 ? '+' : ''}${d.percentage.toFixed(1)}%)`
  ).join('\n')}

  RECOMMENDATION: ${recommendation}
  ${recommendation === 'ADOPT' ? '✅ Experiment shows significant improvement' :
    recommendation === 'REVISE' ? '⚠️ Mixed results - needs refinement' :
...
function generateABTestReport(result: ABTestResult): string {
  ...
  report += result.comparison.pValue < 0.05 ? '✅ Significant\n\n' : '❌ Not Significant\n\n';

  // Recommendation
  report += `## Recommendation: ${result.comparison.recommendation}\n\n`;
  if (result.comparison.recommendation === 'ADOPT') {
    report += '✅ **ADOPT** - The experiment version shows significant improvements with minimal regressions.\n\n';
  } else if (result.comparison.recommendation === 'REVISE') {
    report += '⚠️ **REVISE** - Mixed results. Some improvements but also concerning regressions. Further refinement needed.\n\n';
  } else {
    report += '❌ **REVERT** - The control version performs better or results are not statistically significant.\n\n';
  }
  ...
  report += '\n## Core Principles Summary\n\n';
  const principles = ['cyclicStructure', 'intrinsicMotivation', 'earnedConsequence', 'characterTransformation', 'emotionalResonance'];

  for (const principle of principles) {
    const principleMetrics = Object.values(result.comparison.deltas)
      .filter(d => d.metric.toLowerCase().includes(principle.toLowerCase().replace(/[A-Z]/g, m => m.toLowerCase())));

    if (principleMetrics.length > 0) {
      const avgImprovement = principleMetrics.reduce((sum, d) => sum + d.percentage, 0) / principleMetrics.length;
