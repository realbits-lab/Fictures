...
import { TEST_PROMPTS, getTestPrompt } from './config/test-prompts';
import type { TestStoryResult, TestSuiteResult, EvaluationResult } from './src/types';
...
const EVALUATION_MODE = values.mode as 'quick' | 'standard' | 'thorough' || 'thorough';
...
const OUTPUT_FILE = values.output || `results/${PROMPT_VERSION}/suite-${timestamp}.json`;
 ...
 */
async function generateStory(prompt: string): Promise<{ storyId: string; generationTime: number }> {
  console.log(`  → Generating story: "${prompt.substring(0, 50)}..."`);

  const startTime = Date.now();

  // Get API key from auth file ONLY (not from environment variable)
  const fs = require('fs');
  ...
  if (!apiKey) {
    throw new Error('API key not found in .auth/user.json. Please ensure writer profile exists in develop environment.');
  }

  // Create text generation client with API key and override global
  const { createTextGenerationClient, textGenerationClient } = await import('@/lib/studio/generators/ai-client');
  if (apiKey) {
    const newClient = createTextGenerationClient(apiKey);
    // Override properties of the singleton with the new client
    Object.setPrototypeOf(textGenerationClient, Object.getPrototypeOf(newClient));
    Object.assign(textGenerationClient, newClient);
  }

  // Import generation services
  const { storyService } = await import('@/lib/studio/services/story-service');
  const { characterService } = await import('@/lib/studio/services/character-service');
  const { settingService } = await import('@/lib/studio/services/setting-service');
  const { partService } = await import('@/lib/studio/services/part-service');
  const { chapterService } = await import('@/lib/studio/services/chapter-service');
  const { sceneSummaryService } = await import('@/lib/studio/services/scene-summary-service');
  const { sceneContentService } = await import('@/lib/studio/services/scene-content-service');
  ...
  try {
    ...
    const storyResult = await storyService.generateAndSave({
      ...
      userPrompt: prompt,
      language: 'en',
      apiKey: apiKey  // Pass the API key to the story service
    });
    ...
    const charactersResult = await characterService.generateAndSave({
      userId: 'usr_QKl8WRbF-U2u4ymj', // writer@fictures.xyz user ID
      storyId,
      characterCount: 3,
    ...
    const settingsResult = await settingService.generateAndSave({
      userId: 'usr_QKl8WRbF-U2u4ymj', // writer@fictures.xyz user ID
      storyId,
      settingCount: 3,
      apiKey: apiKey  // Pass the API key
    ...
    const chapter2Result = await chapterService.generateAndSave({
      userId: 'usr_QKl8WRbF-U2u4ymj', // writer@fictures.xyz user ID
      storyId,
      partId: partResult.part.id,
      chapterNumber: 2,
    ...
    const sceneSummariesResult = await sceneSummaryService.generateAndSave({
      userId: 'usr_QKl8WRbF-U2u4ymj', // writer@fictures.xyz user ID
      storyId,
    ...
    for (let i = 0; i < Math.min(3, sceneSummariesResult.scenes.length); i++) {
    ...
    console.log(`    ✓ Story generated in ${(generationTime / 1000).toFixed(1)}s`);
 ...
 */
async function evaluateStory(storyId: string): Promise<TestStoryResult['evaluationResults']> {
  ...
  try {
    ...
    const storyEvalResponse = await fetch(`http://localhost:3000/api/evaluation/story`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ storyId, evaluationMode: EVALUATION_MODE })
    });
    ...
    const charactersEvalResponse = await fetch(`http://localhost:3000/api/evaluation/characters`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ storyId, evaluationMode: EVALUATION_MODE })
    });
    ...
    const settingsEvalResponse = await fetch(`http://localhost:3000/api/evaluation/settings`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ storyId, evaluationMode: EVALUATION_MODE })
    });
    ...
    const { parts, chapters, scenes } = await import('@/lib/schemas/database');
    ...
    if (part) {
      ...
      const partEvalResponse = await fetch(`http://localhost:3000/api/evaluation/part`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ partId: part.id, evaluationMode: EVALUATION_MODE })
      });
      ...
      if (chapter) {
        ...
        const chapterEvalResponse = await fetch(`http://localhost:3000/api/evaluation/chapter`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ chapterId: chapter.id, evaluationMode: EVALUATION_MODE })
        });
        if (chapterEvalResponse.ok) {
          evaluationResults.chapter = await chapterEvalResponse.json();
        }
        ...
        if (scene) {
          ...
          const sceneSummaryEvalResponse = await fetch(`http://localhost:3000/api/evaluation/scene-summary`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ sceneId: scene.id, evaluationMode: EVALUATION_MODE })
          });
          if (sceneSummaryEvalResponse.ok) {
            evaluationResults.sceneSummary = await sceneSummaryEvalResponse.json();
          }

          // Scene content evaluation (if content exists)
          if (scene.content) {
            console.log(`    • Evaluating scene content...`);
            const sceneContentEvalResponse = await fetch(`http://localhost:3000/api/evaluation/scene-content`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ sceneId: scene.id, evaluationMode: EVALUATION_MODE })
            });
            if (sceneContentEvalResponse.ok) {
              evaluationResults.sceneContent = await sceneContentEvalResponse.json();
            }
          }
        }
      }
 ...
 */
function calculateCorePrincipleScores(evaluationResults: TestStoryResult['evaluationResults']): TestStoryResult['corePrincipleScores'] {
  const scores = {
    cyclicStructure: 0,
...
async function main() {
  ...
  for (const promptId of TEST_PROMPT_IDS) {
    ...
    for (let i = 0; i < ITERATIONS; i++) {
      ...
      try {
        // Generate story
        const { storyId, generationTime } = await generateStory(testPrompt.prompt);

        // Evaluate story
        const evaluationResults = await evaluateStory(storyId);

        // Calculate Core Principle scores
        const corePrincipleScores = calculateCorePrincipleScores(evaluationResults);
...
═══════════════════════════════════════════════════════════════
  ...
  Top Issues:
${failurePatterns.slice(0, 3).map(p => `  - ${p.description} (${p.frequency} failures)`).join('\n')}
═══════════════════════════════════════════════════════════════
`);
}
 ...
 */
function aggregateMetrics(results: TestStoryResult[]): TestSuiteResult['aggregatedMetrics'] {
  ...
  for (const result of results) {
    for (const [principle, score] of Object.entries(result.corePrincipleScores)) {
      if (aggregated[principle]) {
        aggregated[principle].cycleCompleteness += score / results.length;
      }
    }
  }
 ...
 */
function identifyFailurePatterns(results: TestStoryResult[]): TestSuiteResult['failurePatterns'] {
    ...
    for (const evalType of Object.values(result.evaluationResults)) {
      if (evalType?.metrics) {
        for (const [metricName, metric] of Object.entries(evalType.metrics)) {
          if (!metric.passed) {
            failureCounts[metricName] = (failureCounts[metricName] || 0) + 1;
          }
        }
      }
    }
  ...
  for (const [metricName, count] of Object.entries(failureCounts)) {
    patterns.push({
      ...
      priority: count > results.length * 0.5 ? 'high' : count > results.length * 0.25 ? 'medium' : 'low'
    });
  }
 ...
 */
function categorizeFailure(metricName: string): TestSuiteResult['failurePatterns'][0]['category'] {
  if (metricName.includes('cycle') || metricName.includes('phase')) return 'structural';
  if (metricName.includes('emotion') || metricName.includes('gamdong')) return 'emotional';
  if (metricName.includes('word') || metricName.includes('prose')) return 'prose';
  if (metricName.includes('seed') || metricName.includes('causal')) return 'causal';
  if (metricName.includes('character') || metricName.includes('arc')) return 'character';
  return 'structural';
}
