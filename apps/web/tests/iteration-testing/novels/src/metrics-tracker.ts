...
export const METRIC_THRESHOLDS = {
  ...
  cycleCompleteness: { target: 1.00, threshold: 0.90 },
  chapterCycleFocus: { target: 1.00, threshold: 1.00 },
  phaseCoverage: { target: 1.00, threshold: 0.90 },
  resolutionAdversityTransition: { target: 0.80, threshold: 0.70 },
  stakesEscalation: { target: 0.85, threshold: 0.75 },
  narrativeMomentum: { target: 0.90, threshold: 0.80 },
  nestedCycleAlignment: { target: 0.85, threshold: 0.75 },
  causalChainContinuity: { target: 1.00, threshold: 0.95 },
  forwardMomentum: { target: 0.90, threshold: 0.80 },

  // Intrinsic Motivation Metrics
  characterActionAuthenticity: { target: 0.90, threshold: 0.80 },
  transactionalLanguageAbsence: { target: 0.85, threshold: 0.75 },
  strategicGoodDeeds: { target: 0.00, threshold: 0.00 },
  rewardExpectationDisplay: { target: 0.00, threshold: 0.00 },
  genuineGoodnessPerception: { target: 0.85, threshold: 0.75 },
  moralElevationTrigger: { target: 0.80, threshold: 0.70 },
  dramaticIrony: { target: 0.90, threshold: 0.80 },

  // Earned Consequence Metrics
  causalConnectionClarity: { target: 0.90, threshold: 0.70 },
  deusExMachinaIncidents: { target: 0.00, threshold: 0.00 },
  temporalSeparation: { target: 0.85, threshold: 0.75 },
  earnedSuccessPerception: { target: 0.80, threshold: 0.70 },
  surprisingButInevitable: { target: 0.85, threshold: 0.75 },
  earnedLuckFeeling: { target: 0.90, threshold: 0.80 },
  seedResolutionRate: { target: 0.70, threshold: 0.50 },
  seedPlantingSuccess: { target: 0.85, threshold: 0.75 },
  seedTrackingCompleteness: { target: 1.00, threshold: 1.00 },
  seedResolutionQuality: { target: 0.90, threshold: 0.80 },
  relationshipSeedDepth: { target: 0.75, threshold: 0.60 },

  // Character Transformation Metrics
  internalFlawDefinition: { target: 1.00, threshold: 1.00 },
  characterDepth: { target: 1.00, threshold: 0.90 },
  flawDrivenAdversity: { target: 0.90, threshold: 0.80 },
  virtueConfrontsFlaw: { target: 0.90, threshold: 0.80 },
  consequenceEnablesGrowth: { target: 0.85, threshold: 0.75 },
  arcProgressionPerception: { target: 0.80, threshold: 0.70 },
  transformationArticulation: { target: 0.75, threshold: 0.60 },
  earnedTransformation: { target: 0.85, threshold: 0.75 },
  arcStructureClarity: { target: 0.90, threshold: 0.80 },

  // Emotional Resonance Metrics
  sceneQualityScore: { target: 3.5 / 4.0, threshold: 3.0 / 4.0 },
  firstPassSuccessRate: { target: 0.85, threshold: 0.70 },
  moralElevationDetection: { target: 0.80, threshold: 0.70 },
  gamdongAchievement: { target: 0.80, threshold: 0.60 },
  empathyBuilding: { target: 0.75, threshold: 0.60 },
  catharsisExperience: { target: 0.70, threshold: 0.50 },
  moralFrameworkClarity: { target: 0.90, threshold: 0.75 },
  emotionalBeatAccuracy: { target: 0.75, threshold: 0.60 },
  emotionalBeatAssignment: { target: 0.75, threshold: 0.60 },
  sceneMemorability: { target: 0.60, threshold: 0.50 },
  settingAmplification: { target: 0.80, threshold: 0.70 },
  wordCountAccuracy: { target: 0.90, threshold: 0.75 },
  formattingCompliance: { target: 1.00, threshold: 0.95 },
 ...
 */
export function aggregateMetrics(stories: TestStoryResult[]): CorePrincipleMetrics {
 ...
 */
function extractAndAddMetrics(story: TestStoryResult, aggregated: CorePrincipleMetrics): void {
  ...
  if (story.evaluationResults.chapter?.metrics) {
    ...
    if (metrics.singleCycleFocus) {
      aggregated.cyclicStructure.cycleCompleteness += metrics.singleCycleFocus.score / 4.0;
      aggregated.cyclicStructure.chapterCycleFocus += metrics.singleCycleFocus.score / 4.0;
    }
    if (metrics.seedTracking) {
      aggregated.earnedConsequence.seedTrackingCompleteness += metrics.seedTracking.score / 4.0;
    }
    if (metrics.stakesEscalation) {
      aggregated.cyclicStructure.stakesEscalation += metrics.stakesEscalation.score / 4.0;
    }
    if (metrics.momentum) {
      aggregated.cyclicStructure.narrativeMomentum += metrics.momentum.score / 4.0;
    }
  }
  ...
  if (story.evaluationResults.characters?.metrics) {
    ...
    if (metrics.characterDepth) {
      aggregated.characterTransformation.characterDepth += metrics.characterDepth.score / 4.0;
      aggregated.intrinsicMotivation.characterActionAuthenticity += metrics.characterDepth.score / 4.0;
    }
    if (metrics.voiceDistinctiveness) {
      aggregated.intrinsicMotivation.dramaticIrony += metrics.voiceDistinctiveness.score / 4.0;
    }
  }
  ...
  if (story.evaluationResults.part?.metrics) {
    ...
    if (metrics.cycleCoherence) {
      aggregated.cyclicStructure.nestedCycleAlignment += metrics.cycleCoherence.score / 4.0;
    }
    if (metrics.conflictDefinition) {
      aggregated.characterTransformation.flawDrivenAdversity += metrics.conflictDefinition.score / 4.0;
    }
    if (metrics.seedTracking) {
      aggregated.earnedConsequence.seedPlantingSuccess += metrics.seedTracking.score / 4.0;
    }
  }
  ...
  if (story.evaluationResults.sceneContent?.metrics) {
    ...
    if (metrics.wordCountCompliance) {
      aggregated.emotionalResonance.wordCountAccuracy += metrics.wordCountCompliance.score / 4.0;
    }
    if (metrics.cycleAlignment) {
      aggregated.cyclicStructure.phaseCoverage += metrics.cycleAlignment.score / 4.0;
    }
    if (metrics.emotionalResonance) {
      aggregated.emotionalResonance.gamdongAchievement += metrics.emotionalResonance.score / 4.0;
      aggregated.emotionalResonance.moralElevationDetection += metrics.emotionalResonance.score / 4.0;
    }
  }
  ...
  if (story.evaluationResults.story?.metrics) {
    ...
    if (metrics.moralFrameworkClarity) {
      aggregated.emotionalResonance.moralFrameworkClarity += metrics.moralFrameworkClarity.score / 4.0;
    }
    if (metrics.thematicCoherence) {
      aggregated.intrinsicMotivation.genuineGoodnessPerception += metrics.thematicCoherence.score / 4.0;
    }
  }
}
 ...
 */
function normalizeMetrics(aggregated: CorePrincipleMetrics, storyCount: number): void {
  for (const principle of Object.keys(aggregated) as Array<keyof CorePrincipleMetrics>) {
 ...
 */
export function calculatePassRates(stories: TestStoryResult[]): Record<string, number> {
  ...
  for (const story of stories) {
    for (const evalType of Object.values(story.evaluationResults)) {
      if (evalType?.metrics) {
        for (const [metricName, metric] of Object.entries(evalType.metrics)) {
  ...
  for (const [metricName, counts] of Object.entries(metricCounts)) {
    passRates[metricName] = counts.total > 0 ? counts.passed / counts.total : 0;
  }
 ...
 */
export function compareMetrics(baseline: MetricSnapshot, experiment: MetricSnapshot): MetricDelta[] {
  ...
  for (const metric of Object.keys(baseline)) {
    ...
    const percentage = baselineValue > 0 ? (delta / baselineValue) * 100 : 0;
...
export function generateMetricReport(
  ...
  const principles = [
    ...
    { name: 'Intrinsic Motivation', metrics: aggregated.intrinsicMotivation },
    { name: 'Earned Consequence', metrics: aggregated.earnedConsequence },
    { name: 'Character Transformation', metrics: aggregated.characterTransformation },
  ...
  for (const principle of principles) {
    ...
    const passRate = calculatePrinciplePassRate(principle.name.toLowerCase().replace(' ', ''), passRates);

    report += `## ${principle.name}\n`;
    report += `- **Average Score**: ${(avgScore * 100).toFixed(1)}%\n`;
    report += `- **Pass Rate**: ${(passRate * 100).toFixed(1)}%\n`;
    report += `- **Status**: ${passRate >= 0.90 ? '✅ Passing' : passRate >= 0.70 ? '⚠️ Warning' : '❌ Failing'}\n\n`;
    ...
    for (const [metricName, value] of Object.entries(principle.metrics)) {
      ...
      const status = value >= threshold.target ? '✅' : value >= threshold.threshold ? '⚠️' : '❌';
 ...
 */
function calculatePrinciplePassRate(principle: string, passRates: Record<string, number>): number {
  const relevantMetrics = Object.keys(passRates).filter(metric => {
    ...
    if (principle.includes('cyclic')) {
      return metric.includes('cycle') || metric.includes('phase') || metric.includes('momentum');
    }
    if (principle.includes('intrinsic')) {
      return metric.includes('authentic') || metric.includes('transactional') || metric.includes('genuine');
    }
    if (principle.includes('earned')) {
      return metric.includes('seed') || metric.includes('causal') || metric.includes('temporal');
    }
    if (principle.includes('transformation')) {
      return metric.includes('flaw') || metric.includes('arc') || metric.includes('transform');
    }
    if (principle.includes('emotional')) {
      return metric.includes('emotion') || metric.includes('gamdong') || metric.includes('moral');
    }
    return false;
  });
  ...
  const totalPassRate = relevantMetrics.reduce((sum, metric) => sum + passRates[metric], 0);
