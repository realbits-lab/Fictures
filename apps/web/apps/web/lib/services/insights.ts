...
async function generateQualityInsights(story: any): Promise<void> {
  const sceneIds = story.chapters.flatMap((ch: any) => ch.scenes.map((sc: any) => sc.id));
  ...
  const allEvaluations = await db
    ...
    .where(sql`${sceneEvaluations.sceneId} IN (${sql.join(sceneIds.map((id: string) => sql`${id}`), sql`, `)})`);

  if (allEvaluations.length === 0) return;

  const avgScores = {
    plot: average(allEvaluations.map((e: any) => parseFloat(e.plotScore))),
    character: average(allEvaluations.map((e: any) => parseFloat(e.characterScore))),
    pacing: average(allEvaluations.map((e: any) => parseFloat(e.pacingScore))),
    prose: average(allEvaluations.map((e: any) => parseFloat(e.proseScore))),
    worldBuilding: average(allEvaluations.map((e: any) => parseFloat(e.worldBuildingScore))),
  };
  ...
  const problemScenes = allEvaluations
    ...
    .sort((a: any, b: any) => {
      const scoreKey = `${lowestCategory.category}Score`;
      return parseFloat(a[scoreKey as keyof typeof a] as string) - parseFloat(b[scoreKey as keyof typeof b] as string);
    })
    .slice(0, 3);
...
async function generateEngagementInsights(story: any): Promise<void> {
  ...
  if (slope < -0.5) {
    const decline = Math.abs((slope * 100) / (engagementData[0]?.views || 1));
...
Format as JSON:
{
  ...
  try {
    ...
    return {
      ...
      actionItems: ['Review your scenes manually for improvements', 'Consider getting feedback from beta readers'],
    };
  }
}
