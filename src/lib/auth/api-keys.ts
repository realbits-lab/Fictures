 ...
 */
export function getExpirationOptions(): Array<{label: string; value: Date | null}> {
  const now = new Date();

  return [
    { label: 'Never', value: null },
    { label: '7 days', value: new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000) },
    { label: '30 days', value: new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000) },
    { label: '90 days', value: new Date(now.getTime() + 90 * 24 * 60 * 60 * 1000) },
    { label: '1 year', value: new Date(now.getTime() + 365 * 24 * 60 * 60 * 1000) },
  ];
}