...
export const updateStory = tool({
  ...
  parameters: z.object({
    ...
    updates: z.object({
      ...
      status: z.enum(['writing', 'published']).optional().describe('Story status'),
      summary: z.string().optional().describe('Story summary and thematic premise'),
      tone: z.enum(['hopeful', 'dark', 'bittersweet', 'satirical']).optional().describe('Emotional tone'),
      moralFramework: z.string().optional().describe('Moral framework and valued virtues'),
    }),
  }),
  execute: async ({ storyId, updates }) => {
    ...
    if (!updated) {
      return { success: false, error: 'Story not found or update failed' };
    }
...
export const createChapter = tool({
  ...
  execute: async ({ storyId, authorId, partId, title, summary, orderIndex }) => {
    const [chapter] = await db
      .insert(chapters)
      .values({
...
export const updateChapter = tool({
  ...
  parameters: z.object({
    ...
    updates: z.object({
      ...
      summary: z.string().optional().describe('Chapter summary'),
      status: z.enum(['writing', 'published']).optional().describe('Chapter status'),
  ...
  execute: async ({ chapterId, updates }) => {
    ...
    if (!updated) {
      return { success: false, error: 'Chapter not found or update failed' };
    }
...
export const updateScene = tool({
  ...
  parameters: z.object({
    ...
    updates: z.object({
      ...
      summary: z.string().optional().describe('Scene summary'),
      orderIndex: z.number().optional().describe('Order index'),
      cyclePhase: z.enum(['setup', 'confrontation', 'virtue', 'consequence', 'transition']).optional(),
      emotionalBeat: z.enum(['fear', 'hope', 'tension', 'relief', 'elevation', 'catharsis', 'despair', 'joy']).optional(),
    }),
  }),
  execute: async ({ sceneId, updates }) => {
    ...
    if (!updated) {
      return { success: false, error: 'Scene not found or update failed' };
    }
...
export const updateSetting = tool({
  ...
  parameters: z.object({
    ...
    updates: z.object({
      ...
      emotionalResonance: z.string().optional().describe('Emotional resonance'),
    }),
  }),
  execute: async ({ settingId, updates }) => {
    ...
    if (!updated) {
      return { success: false, error: 'Setting not found or update failed' };
    }
