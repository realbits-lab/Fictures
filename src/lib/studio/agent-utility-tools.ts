  ...
  execute: async ({ userId, requiredScopes = [] }) => {
    ...
    if (userKeys.length === 0) {
      return {
        ...
        message: 'Please create an API key in Settings with required scopes: ' + requiredScopes.join(', '),
      };
    }
...
export const updatePhaseProgress = tool({
  ...
  parameters: z.object({
    ...
    progressPercent: z.number().optional().describe('Progress percentage (0-100) within the phase'),
  }),
  execute: async ({ chatId, phase, completed, progressPercent }) => {
    try {
      ...
      const currentPhaseIndex = phaseOrder.indexOf(chat.currentPhase || 'story-summary');
      const totalPhases = phaseOrder.length;
      const overallProgress = Math.floor((completedPhases.length / totalPhases) * 100);

      return {
        success: true,
        message: completed ? `Phase ${phase} completed` : `Phase ${phase} in progress`,
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to update phase progress',
      };
    }
  },
});
