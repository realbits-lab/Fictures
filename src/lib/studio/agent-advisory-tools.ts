  ...
  execute: async ({ storyId, targetPhase }) => {
    ...
    switch (targetPhase) {
      case 'story-summary':
        ...
        suggestions.push('Provide a story concept with genre, themes, and tone');
        break;
      ...
      case 'parts':
        ...
        if (characterCount.count === 0) {
          prerequisitesMet = false;
          missingPrerequisites.push('At least one character is required');
        ...
        if (settingCount.count === 0) {
          prerequisitesMet = false;
          missingPrerequisites.push('At least one setting is required');
      ...
      case 'scene-summaries':
        ...
        if (chapterCount.count === 0) {
          prerequisitesMet = false;
          missingPrerequisites.push('At least one chapter is required');
      ...
      case 'scene-content':
        ...
        if (sceneCount.count === 0) {
          prerequisitesMet = false;
          missingPrerequisites.push('At least one scene summary is required');
...
export const suggestNextPhase = tool({
  ...
  execute: async ({ storyId }) => {
    ...
    if (!hasStorySummary) {
      ...
      reason = 'Story summary is ready - generate character profiles next';
    } else if (!hasSettings) {
      nextPhase = 'settings';
      reason = 'Characters created - generate story settings next';
    } else if (!hasParts) {
      nextPhase = 'parts';
      reason = 'Foundation is ready - generate story parts structure next';
    } else if (!hasChapters) {
      nextPhase = 'chapters';
      ...
      reason = 'Scene summaries created - generate full scene content next';
    }
