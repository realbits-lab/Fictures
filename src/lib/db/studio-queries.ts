...
import { stories, chapters, parts } from './schema';
...
const CACHE_KEYS = {
  ...
  userStory: (storyId: string, userId: string) => `fictures:studio:story:${storyId}:user:${userId}`,
...
export async function getCachedUserStories(userId: string) {
  ...
  console.log(`[StudioCache] ❌ MISS: User stories for ${userId} - fetching from DB`);
  ...
  if (userStories.length === 0) {
    ...
    console.log(`[StudioCache] 💾 SET: User stories for ${userId} (0 stories, TTL: ${CACHE_TTL.STUDIO_DRAFT_LIST}s)`);
    return result;
  }
  ...
  console.log(`[StudioCache] 💾 SET: User stories for ${userId} (${storiesWithData.length} stories, TTL: ${CACHE_TTL.STUDIO_DRAFT_LIST}s, ${duration}ms)`);
...
export async function getCachedStoryDetail(storyId: string, userId: string) {
  ...
  const cached = await cache.get(cacheKey);
  if (cached) {
    console.log(`[StudioCache] ✅ HIT: Story detail ${storyId} (${isPublished ? 'published' : 'draft'})`);
    return cached;
  }

  console.log(`[StudioCache] ❌ MISS: Story detail ${storyId} - fetching from DB`);

  // Cache the full story data (includes imageVariants for detail view)
  await cache.set(cacheKey, story[0], cacheTTL);
  console.log(`[StudioCache] 💾 SET: Story detail ${storyId} (TTL: ${cacheTTL}s)`);
...
export async function invalidateStudioCache(userId: string, storyId?: string) {
  ...
  console.log(`[StudioCache] 🗑️  INVALIDATED: ${keysToInvalidate.length} keys for user ${userId}`);
}
