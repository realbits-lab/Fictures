...
import { stories, users, communityPosts, characters, settings } from './schema';
  ...
  try {
    ...
    const [publishedStories, authorData, postsCounts] = await Promise.all([
      ...
      db.select({
        ...
        lastActivity: sql<Date>`MAX(${communityPosts.createdAt})`.as('lastActivity'),
      })
      .from(communityPosts)
    ...
    console.log(`[PERF-QUERY] ⚡ Batched query (3 queries in parallel): ${Math.round(queryEnd - queryStart)}ms`);

    // Assemble data in memory (fast)
    const authorsMap = new Map(authorData.map(author => [author.id, author]));
    const postsMap = new Map(postsCounts.map(p => [p.storyId, { count: Number(p.totalPosts), lastActivity: p.lastActivity }]));
    ...
    const storiesWithStats = publishedStories.map(story => {
      ...
      const lastActivityDate = postData?.lastActivity || story.updatedAt || story.createdAt;
      ...
      return {
        ...
        isActive,
        lastActivity: lastActivityDate?.toISOString() || story.createdAt.toISOString(),
      };
    });

    const totalDuration = Math.round(performance.now() - queryStart);
    console.log(`[PERF-QUERY] 🏁 getCommunityStoriesForReading COMPLETE: ${totalDuration}ms`);
    ...
    console.error(`[PERF-QUERY] ❌ getCommunityStoriesForReading FAILED after ${errorDuration}ms:`, error);
    throw error;
  }
}
...
export async function getCommunityStoryForReading(storyId: string) {
  ...
  try {
    const [storyResult, storyCharacters, storySettings, postStats] = await Promise.all([
    ...
    const queryEnd = performance.now();
    console.log(`[PERF-QUERY] ⚡ Batched query (4 queries in parallel): ${Math.round(queryEnd - queryStart)}ms`);
    ...
    const totalDuration = Math.round(performance.now() - queryStart);
    console.log(`[PERF-QUERY] 🏁 getCommunityStoryForReading COMPLETE: ${totalDuration}ms`);
    console.log(`[PERF-QUERY] 📊 Results: ${storyCharacters.length} characters, ${storySettings.length} settings`);

    return result;
  } catch (error) {
    const errorDuration = Math.round(performance.now() - queryStart);
    console.error(`[PERF-QUERY] ❌ getCommunityStoryForReading FAILED after ${errorDuration}ms:`, error);
    throw error;
  }
}
...
export async function getCommunityPostsForReading(storyId: string) {
  ...
  try {
    ...
    const totalDuration = Math.round(performance.now() - queryStart);
    console.log(`[PERF-QUERY] 🏁 getCommunityPostsForReading COMPLETE: ${totalDuration}ms`);
    ...
    const errorDuration = Math.round(performance.now() - queryStart);
    console.error(`[PERF-QUERY] ❌ getCommunityPostsForReading FAILED after ${errorDuration}ms:`, error);
    throw error;
  }
}
