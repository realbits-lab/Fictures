...
export function useRoleProtectedAction(
  ...
  const hasRequiredRole = session?.user?.role && requiredRoles.includes(session.user.role);
  ...
  const executeAction = () => {
    if (!session) {
      ...
      alert(`This action requires one of the following roles: ${requiredRoles.join(', ')}`);
    } else {
