...
export function AdvancedCacheMetricsDashboard() {
  ...
  const { data, error, mutate } = useSWR<MetricsResponse>(
    isVisible ? `/studio/api/cache/metrics?timeRange=${timeRange}&groupBy=${groupBy}` : null,
    fetcher,
    {
  ...
  const exportAsCSV = () => {
    ...
    const headers = ['Timestamp', 'Cache Type', 'Operation', 'Key', 'Duration (ms)'];
  ...
  if (error) {
    return (
      <div className="fixed bottom-4 right-4 z-50 w-[800px] bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-700 rounded-lg p-4">
        <p className="text-red-700 dark:text-red-300">Failed to load metrics: {error.message}</p>
        <button
        ...
        <CardContent className="text-xs space-y-4">
          ...
          <div className="flex gap-4 items-center border-b border-gray-200 dark:border-gray-700 pb-3">
            <div className="flex gap-2 items-center">
              <label className="text-gray-600 dark:text-gray-400">Time Range:</label>
              <select
                value={timeRange}
            ...
            <div className="flex gap-2 items-center">
              <label className="text-gray-600 dark:text-gray-400">Group By:</label>
              <select
                value={groupBy}
              ...
              {Object.entries(data.byType).map(([type, stats]) => (
                <div key={type} className="p-3 bg-gray-50 dark:bg-gray-800 rounded">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs text-gray-600 dark:text-gray-400">
                    <div>
                      <span className="text-green-600">Hits:</span> {formatNumber(stats.hits)}
                    </div>
                    <div>
                      <span className="text-orange-600">Misses:</span> {formatNumber(stats.misses)}
                    </div>
                    <div>
              ...
              {data.recentOperations.map((op, index) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2 items-center">
                      {op.duration && (
                        <span className="text-gray-500">{formatDuration(op.duration)}</span>
                      )}
          ...
          <div className="text-xs text-gray-500 dark:text-gray-400 border-t border-gray-200 dark:border-gray-700 pt-2">
            <div>
              Press <kbd className="px-1 bg-gray-200 dark:bg-gray-700 rounded">Ctrl+Shift+M</kbd>{' '}
              to toggle
            </div>
            <div className="text-xs text-gray-400 mt-1">Auto-refreshes every 5 seconds</div>
          </div>
        </CardContent>
      </Card>
    </div>
