...
import { ArrowLeft, TrendingUp, Users, Eye, Heart, Clock, Target, Star } from "lucide-react";
...
export function StoryAnalyticsDashboard({ storyId }: StoryAnalyticsDashboardProps) {
  const { data: session } = useSession();
  const [timeRange, setTimeRange] = useState<'7d' | '30d' | '90d'>('30d');

  const { data: analytics, isLoading: analyticsLoading, error: analyticsError } = useSWR(
    session?.user?.id ? `/analysis/api/story/${storyId}?range=${timeRange}` : null,
    fetcher,
    {
  ...
  const { data: dailyData, isLoading: dailyLoading } = useSWR(
    session?.user?.id ? `/analysis/api/daily?storyId=${storyId}&range=${timeRange}` : null,
    fetcher,
    {
      revalidateOnFocus: false,
      revalidateOnReconnect: false,
        ...
        <SkeletonLoader>
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="h-32 bg-[rgb(var(--color-muted))] rounded-lg animate-pulse"></div>
              ))}
            </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center space-y-4">
          ...
          <p className="text-[rgb(var(--color-muted-foreground))]">
            {analyticsError?.message || 'An error occurred while fetching analytics data.'}
          </p>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <MetricCard
          ...
          trend={analytics.trends.viewsChange > 0 ? 'up' : analytics.trends.viewsChange < 0 ? 'down' : 'neutral'}
        ...
        <MetricCard
          ...
          trend={analytics.trends.readersChange > 0 ? 'up' : analytics.trends.readersChange < 0 ? 'down' : 'neutral'}
        ...
        <MetricCard
          ...
          trend={analytics.trends.engagementChange > 0 ? 'up' : analytics.trends.engagementChange < 0 ? 'down' : 'neutral'}
      ...
      <div className="bg-[rgb(var(--color-card))] border border-[rgb(var(--color-border))] rounded-lg p-6 mb-8">
        ...
        {dailyLoading ? (
          <div className="h-80 flex items-center justify-center">
            <p className="text-[rgb(var(--color-muted-foreground))]">Loading chart...</p>
          </div>
        ) : daily.length === 0 ? (
          <div className="h-80 flex items-center justify-center">
            <p className="text-[rgb(var(--color-muted-foreground))]">
          ...
          <LineChart
            ...
            lines={[
              ...
              { key: 'readers', name: 'Unique Readers', color: '#10b981' },
              { key: 'engagement', name: 'Engagements', color: '#f59e0b' },
            ]}
            height={320}
          />
        )}
      ...
      <div className="bg-[rgb(var(--color-card))] border border-[rgb(var(--color-border))] rounded-lg p-6 mb-8">
        ...
        <BarChart
          data={[
            { category: 'Plot', score: analytics.quality.scoresByCategory.plot },
            { category: 'Character', score: analytics.quality.scoresByCategory.character },
            { category: 'Pacing', score: analytics.quality.scoresByCategory.pacing },
            { category: 'Prose', score: analytics.quality.scoresByCategory.prose },
            { category: 'World-Building', score: analytics.quality.scoresByCategory.worldBuilding },
          ]}
          xKey="category"
          bars={[
            { key: 'score', name: 'Quality Score', color: '#8b5cf6' },
          ]}
          height={300}
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-[rgb(var(--color-card))] border border-[rgb(var(--color-border))] rounded-lg p-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="text-sm font-medium text-[rgb(var(--color-muted-foreground))]">Comments</h3>
          </div>
        ...
        <div className="bg-[rgb(var(--color-card))] border border-[rgb(var(--color-border))] rounded-lg p-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="text-sm font-medium text-[rgb(var(--color-muted-foreground))]">Likes</h3>
          </div>
          <p className="text-3xl font-bold text-[rgb(var(--color-foreground))]">
        ...
        <div className="bg-[rgb(var(--color-card))] border border-[rgb(var(--color-border))] rounded-lg p-6">
          <div className="flex items-center gap-3 mb-2">
            <div className="text-2xl">📤</div>
            <h3 className="text-sm font-medium text-[rgb(var(--color-muted-foreground))]">Shares</h3>
          </div>
          <p className="text-3xl font-bold text-[rgb(var(--color-foreground))]">
