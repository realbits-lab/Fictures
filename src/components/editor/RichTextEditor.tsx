...
function convertHtmlToMarkdown(html: string): string {
  ...
  // Lists
  markdown = markdown.replace(/<ul>([\s\S]*?)<\/ul>/g, (_match: string, content: string) => {
    return content.replace(/<li>(.*?)<\/li>/g, '- $1\n');
  });
  markdown = markdown.replace(/<ol>([\s\S]*?)<\/ol>/g, (_match: string, content: string) => {
    let counter = 1;
    return content.replace(/<li>(.*?)<\/li>/g, (_m: string, item: string) => {
      return `${counter++}. ${item}\n`;
    });
  });

  // Blockquotes
  markdown = markdown.replace(/<blockquote>([\s\S]*?)<\/blockquote>/g, (_match: string, content: string) => {
