...
export function JSONDataDisplay({
  ...
  const renderStoryJSON = () => {
    ...
    return (
      <div className="space-y-3">
        {renderJSONSection("Story Foundation", {
        ...
        {renderJSONSection("Story Structure", {
          themes: storyData.themes,
          structure: storyData.structure,
          parts: storyData.parts
        }, "structure")}

        {renderJSONSection("Setting & World", storyData.setting, "setting")}

        {renderJSONSection("Serial Publication", {
  ...
  const renderPartJSON = () => {
    ...
    return (
      <div className="space-y-3">
        {renderJSONSection("Part Overview", {
          part: partData.part,
        ...
        {renderJSONSection("Central Questions", partData.questions, "questions")}

        {renderJSONSection("Character Development", partData.chars, "chars")}

        {renderJSONSection("Plot Progression", partData.plot, "plot")}

        {renderJSONSection("Thematic Focus", partData.themes, "themes")}

        {renderJSONSection("Emotional Journey", partData.emotion, "emotion")}
  ...
  const renderChapterJSON = () => {
    ...
    return (
      <div className="space-y-3">
        {renderJSONSection("Chapter Overview", {
        ...
        {renderJSONSection("Three-Act Structure", chapterData.acts, "acts")}

        {renderJSONSection("Character Arcs", chapterData.chars, "chars")}

        {renderJSONSection("Tension Architecture", chapterData.tension, "tension")}

        {renderJSONSection("Dual Mandate", chapterData.mandate, "mandate")}
  ...
  const renderSceneJSON = () => {
    ...
    return (
      <div className="space-y-3">
        {renderJSONSection("Scene Overview", {
          id: sceneData.id,
        ...
        {renderJSONSection("Characters", sceneData.characters, "characters")}

        {renderJSONSection("Scene Structure", {
          goal: sceneData.goal,
        ...
        {renderJSONSection("Scene Beats", {
          beats: sceneData.beats,
          leads_to: sceneData.leads_to
        }, "beats")}

        {renderJSONSection("Visual Description", {
  ...
  const getLevelTitle = () => {
    switch (currentLevel) {
      case "story": return "Story JSON Data";
      case "part": return "Story JSON Data"; // Changed from "Part JSON Data" to "Story JSON Data"
      case "chapter": return "Chapter JSON Data";
      case "scene": return "Scene JSON Data";
  ...
  const getMainTitle = () => {
    switch (currentLevel) {
      case "part": return "Story Data"; // Changed from "JSON Data" to "Story Data"
        ...
        <div className="space-y-3">
          ...
          {!storyData && !partData && !chapterData && !sceneData && (
            <div className="text-center py-8 text-gray-500">
              <p className="text-sm">No JSON data available for this {currentLevel}</p>
