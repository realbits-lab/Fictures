...
export function FormatDistribution({
  ...
  return (
    <div className={`space-y-2 ${className}`}>
        ...
        <div className={`flex items-center justify-between ${textSizeClasses[size]}`}>
          ...
          {showPercentages && (
            <span className="text-gray-600 dark:text-gray-400">
              {novelPercentage.toFixed(1)}% ({novelViews.toLocaleString()})
            </span>
          )}
        </div>
      )}
      ...
      {/* Comic Bar */}
      {showLabels && (
        <div className={`flex items-center justify-between ${textSizeClasses[size]} mt-3`}>
          <span className="text-purple-600 dark:text-purple-400 font-medium flex items-center gap-1">
            🎨 Comic
          </span>
          {showPercentages && (
            <span className="text-gray-600 dark:text-gray-400">
              {comicPercentage.toFixed(1)}% ({comicViews.toLocaleString()})
            </span>
          )}
        </div>
      )}
  ...
  return (
    <div className={`flex items-center gap-1 ${className}`}>
      ...
      <div className="text-xs text-gray-600 dark:text-gray-400 whitespace-nowrap">
        <span className="text-blue-600 dark:text-blue-400">{novelPercentage.toFixed(0)}%</span>
        {' / '}
        <span className="text-purple-600 dark:text-purple-400">{comicPercentage.toFixed(0)}%</span>
      </div>
    </div>
  );
}
