  ...
  try {
    ...
    if (body.messages && Array.isArray(body.messages) && body.messages.length > 0) {
      ...
      if (lastMessage && typeof lastMessage === 'object' && lastMessage.content) {
        ...
        return new Response(
          JSON.stringify({ error: 'Invalid request format: message has no content' }),
          { status: 400, headers: { 'Content-Type': 'application/json' } }
        );
      }
    } else if (body.message && typeof body.message === 'object' && body.message.content) {
      ...
      return new Response(
        JSON.stringify({ error: 'Invalid request format: missing message content' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    let chat = (chatId && isValidUUID(chatId)) ? await getStudioAgentChat(chatId) : null;
    if (!chat) {
