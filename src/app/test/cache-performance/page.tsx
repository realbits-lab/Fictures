...
export default function CachePerformancePage() {
  ...
  const {
    ...
    '/test/cache-performance/api/stories',
    fetcher,
    {
  ...
  const measurePerformance = async (
    ...
    console.log(`[${phase}] Duration: ${duration.toFixed(2)}ms, Source: ${source}, Cache Hit: ${cacheHit}`);
  ...
  const runCacheTest = async () => {
    ...
    try {
      ...
      const coldLoad = await measurePerformance('Cold Load', async () => {
        const response = await fetch(`/test/cache-performance/api/stories/${testStoryId}`);
      ...
      setCurrentPhase('Phase 3: Updating data (testing cache invalidation)');
      await new Promise((resolve) => setTimeout(resolve, 1000));
      ...
      await new Promise((resolve) => setTimeout(resolve, 1000));
      const afterUpdate = await measurePerformance('After Update', async () => {
        const response = await fetch(`/test/cache-performance/api/stories/${testStoryId}`);
        return { data: await response.json(), headers: response.headers };
      });
      ...
      <div className="bg-white rounded-lg shadow-lg p-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Tests all three caching layers: SWR Memory (30min) → localStorage (1hr) → Redis (10min)
          </p>
        </div>
              ...
              {stories.map((story) => (
                ...
                >
                  ...
                  <div className="text-sm text-gray-600 mt-2">
                    ...
                    <p>Chapters: {story.chaptersCount} | Scenes: {story.scenesCount}</p>
        ...
        <div className="mb-8">
          ...
          <div className="flex gap-4">
            ...
            >
              {isTestRunning ? 'Running Test...' : 'Run Full Cache Test'}
            </button>
            <button
              ...
              <table className="min-w-full border-collapse border border-gray-300">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th className="border border-gray-300 px-4 py-2">Duration (ms)</th>
                    <th className="border border-gray-300 px-4 py-2">Source</th>
                    <th className="border border-gray-300 px-4 py-2">Cache Hit</th>
                ...
                <tbody>
                  {testResults.coldLoad && (
                    <tr>
                      <td className="border border-gray-300 px-4 py-2">Cold Load</td>
                      <td className="border border-gray-300 px-4 py-2 text-right">
                        {testResults.coldLoad.duration.toFixed(2)}
                      </td>
                      ...
                      <td className="border border-gray-300 px-4 py-2">
                        {getPerformanceRating(testResults.coldLoad.duration, 'cold')}
                      </td>
                    </tr>
                  )}
                  {testResults.warmLoad && (
                    <tr>
                      <td className="border border-gray-300 px-4 py-2">Warm Load</td>
                      <td className="border border-gray-300 px-4 py-2 text-right font-semibold text-green-600">
                        {testResults.warmLoad.duration.toFixed(2)}
                      </td>
                      <td className="border border-gray-300 px-4 py-2">
                      ...
                      <td className="border border-gray-300 px-4 py-2">
                        {getPerformanceRating(testResults.warmLoad.duration, 'warm')}
                      </td>
                    </tr>
                  )}
                  {testResults.afterUpdate && (
                    <tr>
                      <td className="border border-gray-300 px-4 py-2">After Update</td>
                      <td className="border border-gray-300 px-4 py-2 text-right">
                        {testResults.afterUpdate.duration.toFixed(2)}
                      </td>
                      <td className="border border-gray-300 px-4 py-2">
                        {testResults.afterUpdate.source}
                      </td>
                      <td className="border border-gray-300 px-4 py-2">
                        {testResults.afterUpdate.cacheHit ? '✅' : '❌'}
                      </td>
                      <td className="border border-gray-300 px-4 py-2">
                        {getPerformanceRating(testResults.afterUpdate.duration, 'update')}
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            {testResults.coldLoad && testResults.warmLoad && (
              <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                ...
                <p className="text-sm">
                  ...
                  {(testResults.coldLoad.duration / testResults.warmLoad.duration).toFixed(2)}x faster
                </p>
                <p className="text-sm">
                  <strong>Time Saved:</strong>{' '}
                  {(testResults.coldLoad.duration - testResults.warmLoad.duration).toFixed(2)}ms
                </p>
                <p className="text-sm">
                  <strong>Improvement:</strong>{' '}
                  {(
                    ((testResults.coldLoad.duration - testResults.warmLoad.duration) /
        ...
        <div className="mb-8">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div className="border border-gray-200 rounded-lg p-4">
              <h3 className="font-semibold mb-2">Layer 1: SWR Memory</h3>
              <p className="text-sm text-gray-600">TTL: 30 minutes</p>
              <p className="text-sm text-gray-600">Location: Browser memory</p>
              <p className="text-sm text-gray-600">Expected: {'<'}1ms</p>
            </div>
            <div className="border border-gray-200 rounded-lg p-4">
              <h3 className="font-semibold mb-2">Layer 2: localStorage</h3>
              <p className="text-sm text-gray-600">TTL: 1 hour</p>
              <p className="text-sm text-gray-600">Location: Browser storage</p>
              <p className="text-sm text-gray-600">Expected: 5-20ms</p>
            </div>
            <div className="border border-gray-200 rounded-lg p-4">
              <h3 className="font-semibold mb-2">Layer 3: Redis</h3>
              <p className="text-sm text-gray-600">TTL: 10 minutes</p>
              <p className="text-sm text-gray-600">Location: Server cache</p>
              <p className="text-sm text-gray-600">Expected: 40-70ms</p>
            </div>
          </div>
        </div>
      </div>
