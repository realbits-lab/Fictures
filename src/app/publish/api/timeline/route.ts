...
export async function GET(request: NextRequest) {
  try {
    ...
    conditions.push(sql`${scheduledPublications.storyId} IN (${sql.join(storyIds.map(id => sql`${id}`), sql`, `)})`);
    ...
    if (startDate) {
      conditions.push(gte(scheduledPublications.scheduledFor, new Date(startDate)));
    }

    if (endDate) {
      conditions.push(lte(scheduledPublications.scheduledFor, new Date(endDate)));
    }
