    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          const hnsDoc: HNSDocument = await generateCompleteHNS(prompt, language);
          ...
          const [createdStory] = await db.insert(stories).values({
            ...
            partIds: hnsDoc.story.parts,
            chapterIds: hnsDoc.chapters.map(c => c.chapter_id),
          }).returning();

          // Create parts
          for (const part of hnsDoc.parts) {
          ...
          const characterImagePromises = hnsDoc.characters.map(async (character) => {
            try {
              ...
              await db.insert(charactersTable).values({
                ...
                name: character.name,
                storyId: storyId,
                isMain: ['protagonist', 'antagonist'].includes(character.role),
                role: character.role,
                ...
                physicalDescription: character.physical_description as Record<string, unknown>,
              ...
              return { characterId: character.character_id, name: character.name, imageUrl };
            } catch (error) {
          ...
          const settingImagePromises = hnsDoc.settings.map(async (setting) => {
            try {
              ...
              return { settingId: setting.setting_id, name: setting.name, imageUrl };
            } catch (error) {
