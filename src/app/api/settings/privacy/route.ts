...
export async function PUT(request: Request) {
  try {
    ...
    const updatedSettings = {
      ...
      activity: {
        showReadingActivity: body.activity?.showReadingActivity ?? false,
      ...
      data: {
        ...
        shareDataWithPartners: body.data?.shareDataWithPartners ?? false,
        dataRetentionPeriod: body.data?.dataRetentionPeriod || '2-years'
      },
      communication: {
        allowDirectMessages: body.communication?.allowDirectMessages ?? true,
        allowMentions: body.communication?.allowMentions ?? true,
        restrictDMsToFollowers: body.communication?.restrictDMsToFollowers ?? false,
        blockUnverifiedUsers: body.communication?.blockUnverifiedUsers ?? false
      },
      safety: {
        enableTwoFactorAuth: body.safety?.enableTwoFactorAuth ?? false,
        requirePasswordForSensitiveActions: body.safety?.requirePasswordForSensitiveActions ?? true,
