// Test with the exact YAML being sent from the browser

const browserStoryData = {
...
// Convert to the exact YAML format the frontend creates
import * as yaml from 'js-yaml';

const exactYaml = yaml.dump({ story: browserStoryData }, { indent: 2 });

async function testExactBrowserYaml() {
  console.log('🧪 Testing with exact browser YAML format...\n');
  console.log('📄 YAML Length:', exactYaml.length);
  console.log('📄 YAML Preview (first 300 chars):\n', exactYaml.substring(0, 300) + '...\n');
  ...
  try {
    ...
    if (result.toolsUsed) {
      ...
      console.log('❌ NO TOOLS CALLED - This is the problem!');
    }
    ...
    // Try with a more specific request
    console.log('\n🔄 Trying with more specific request...');

    const response2 = await fetch('http://localhost:3000/api/story-analyzer', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        storyYaml: exactYaml,
        userRequest: 'Change the title of this story to something more exciting'
      })
    });

    const result2 = await response2.json();
    console.log('📊 Specific Request - Success:', result2.success);
    if (result2.toolsUsed) {
      console.log('🔧 Specific Request - Tools Used:', result2.toolsUsed);
    }
    if (result2.error) {
      console.log('🚨 Specific Request - Error:', result2.error);
    }
